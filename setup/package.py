...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'leewise', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/leewise && \
    groupadd -g %(group_id)s leewise && \
    useradd -u %(user_id)s -g leewise leewise -d /var/lib/leewise && \
    mkdir /data && \
    chown leewise:leewise /var/lib/leewise /data
USER leewise
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class LeewiseTestTimeoutError(Exception):
    pass


class LeewiseTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise LeewiseTestError("Installation of package failed")
        else:
        ...
        raise LeewiseTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/leewise_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.leewise_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'leewise/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'leewise-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='leewise', exposed_port=None, detach=False, timeout=None):
    ...
    def test_leewise(self):
        logging.info('Starting to test Leewise install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'leewise.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise LeewiseTestTimeoutError('Leewise pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise LeewiseTestError('Error while installing/starting Leewise after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'leewise-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/leewise-*.tar.gz' % self.args.build_dir)[0], '%s/leewise_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/leewise-*.zip' % self.args.build_dir)[0], '%s/leewise_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s leewise"',
            'su leewise -s /bin/bash -c "python3 -m venv /var/lib/leewise/leewisevenv"',
            'su leewise -s /bin/bash -c "/var/lib/leewise/leewisevenv/bin/python3 -m pip install --upgrade pip"',
            'su leewise -s /bin/bash -c "/var/lib/leewise/leewisevenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su leewise -s /bin/bash -c "/var/lib/leewise/leewisevenv/bin/python3 -m pip install /data/src/leewise_{VERSION}.{TSTAMP}.tar.gz"',
            'su leewise -s /bin/bash -c "createdb mycompany"',
            'su leewise -s /bin/bash -c "/var/lib/leewise/leewisevenv/bin/leewise -d mycompany -i base --stop-after-init"',
            'su leewise -s /bin/bash -c "/var/lib/leewise/leewisevenv/bin/leewise -d mycompany --pidfile=/data/src/leewise.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'leewise-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_leewise()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/leewise (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../leewise_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'leewise-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/leewise_{VERSION}.{TSTAMP}_all.deb',
            'su leewise -s /bin/bash -c "leewise -d mycompany -i base --pidfile=/data/src/leewise.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'leewise-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_leewise()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/leewise/rpmbuild'
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/leewise.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./leewise-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/leewise-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" /data/src/setup/rpm/leewise.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/leewise*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'leewise-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/leewise-*.noarch.rpm' % self.args.build_dir)[0], '%s/leewise_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s leewise"',
            'su leewise -c "createdb mycompany"',
            'dnf install -d 0 -e 0 /data/src/leewise_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su leewise -s /bin/bash -c "leewise -c /etc/leewise/leewise.conf -d mycompany -i base --stop-after-init"',
            'su leewise -s /bin/bash -c "leewise -c /etc/leewise/leewise.conf -d mycompany --pidfile=/data/src/leewise.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'leewise-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_leewise()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'leewise-rpm-createrepo-%s' % TSTAMP)
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/leewisebuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing Leewise files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setup_path = glob("%s/leewise_setup_*.exe" % self.args.build_dir)[0]
        setupfile = setup_path.split('/')[-1]
        setupversion = setupfile.split('leewise_setup_')[1].split('.exe')[0]

        self.rsync(['%s' % setup_path, '%s@127.0.0.1:' % self.login])
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Leewise %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"Leewise {sv}"/python/python.exe \'c:\\Program Files\\Leewise {sv}\\server\\leewise-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/leewise/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/leewise/vm/win1036/id_rsa', help="%(default)s")
    ...
    parsed_args.leewise_dir = ROOTDIR
    return parsed_args
