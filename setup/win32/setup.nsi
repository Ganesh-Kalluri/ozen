...
!ifndef SERVICENAME
	!define SERVICENAME 'leewise-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\leewisebuild'
!endif

!define PRODUCT_NAME "Leewise"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Leewise Server ${VERSION}"
...
OutFile "leewise_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\leewise-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\leewise-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\leewise-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\leewise-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.leewise.in/page/contactus"
...
; English
LangString DESC_Leewise_Server ${LANG_ENGLISH} "Install the Leewise Server with all the Leewise standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Leewise."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Leewise for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Leewise Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "Leewise Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "Leewise Server Only"
LangString Profile_IOT ${LANG_ENGLISH} "Leewise IoT"
LangString TITLE_Leewise_Server ${LANG_ENGLISH} "Leewise Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString TITLE_IOT ${LANG_ENGLISH} "Leewise IoT"
LangString TITLE_Nginx ${LANG_ENGLISH} "Nginx WebServer"
LangString TITLE_Ghostscript ${LANG_ENGLISH} "Ghostscript interpreter"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Leewise"

; French
LangString DESC_Leewise_Server ${LANG_FRENCH} "Installation du Serveur Leewise avec tous les modules Leewise standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Leewise."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Leewise pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Leewise"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur Leewise Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur Leewise"
LangString Profile_IOT ${LANG_FRENCH} "Leewise IoT"
LangString TITLE_Leewise_Server ${LANG_FRENCH} "Serveur Leewise"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString TITLE_IOT ${LANG_FRENCH} "Leewise IoT"
...
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Leewise"
...
Section $(TITLE_Leewise_Server) SectionLeewise_Server
    SectionIn 1 2 3
    ...
    # If there is a previous install of the Leewise Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "addons_path" "$INSTDIR\server\leewise\addons"
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    # Set data_dir
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "data_dir" "$INSTDIR\sessions"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\leewise.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\leewise-bin" --stop-after-init -c "$INSTDIR\server\leewise.conf" --logfile "$INSTDIR\server\leewise.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\leewise-bin\" -c "\"$INSTDIR\server\leewise.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    Call RestartLeewiseService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_Leewise" \
...
Section $(TITLE_IOT) IOT
    ...
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "server_wide_modules" "web,hw_posbox_homepage,hw_drivers"
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\server\leewise.conf" "options" "max_cron_threads" "0"
    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\server\leewise-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
    Call RestartLeewiseService
SectionEnd
...
Section $(TITLE_Nginx) Nginx
    ...
    File "..\..\leewise\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\leewise\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@leewise.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.leewise.in"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@leewise.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionLeewise_Server} $(DESC_Leewise_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Leewise ${VERSION}"
...
Function ShowProxyTokenDialogPage
    ...
    doProxyToken:
        ...
        ${NSD_CreateLabel} 0 0 100% 25% "Here is your access token for the Leewise IOT, please write it down in a safe place, you will need it to configure the IOT"
        Pop $ProxyTokenLabel
...
Function ComponentLeave
    SectionGetFlags ${SectionLeewise_Server} $0
...
Function RestartLeewiseService
    DetailPrint "Restarting Leewise Service"
