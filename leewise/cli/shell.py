...
class Shell(Command):
    """Start leewise in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        config.parse_config(args)
        leewise.cli.server.report_configuration()
        leewise.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            registry = leewise.registry(dbname)
            with registry.cursor() as cr:
                uid = leewise.SUPERUSER_ID
                ctx = leewise.api.Environment(cr, uid, {})['res.users'].context_get()
                env = leewise.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
