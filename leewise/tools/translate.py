...
PYTHON_TRANSLATION_COMMENT = 'leewise-python'

# translation used for javascript code in web client
JAVASCRIPT_TRANSLATION_COMMENT = 'leewise-javascript'
# used to notify web client that these translations should be loaded in the UI
# deprecated comment since Leewise 16.0
...
class GettextAlias(object):
    ...
    def _get_db(self):
        ...
        if db_name:
            return leewise.sql_db.db_connect(db_name)
    ...
    def _get_lang(self, frame):
        ...
        if not lang:
            ...
            if not lang:
                ...
                if cr and uid:
                    env = leewise.api.Environment(cr, uid, {})
        ...
        try:
            ...
            if lang and lang != 'en_US':
                if not module:
                    ...
                    path_info = leewise.modules.get_resource_from_path(path)
...
def TranslationFileReader(source, fileformat='po'):
    """ Iterate over translation file to return Leewise translation entries """
    if fileformat == 'csv':
...
class PoFileReader:
    """ Iterate over po file to return Leewise translation entries """
    def __init__(self, source):
...
def _extract_translatable_qweb_terms(element, callback):
    ...
    for el in element:
        ...
        if (el.tag.lower() not in SKIPPED_ELEMENTS
            ...
            # https://github.com/leewise/owl/blob/master/doc/reference/component.md#composition
...
class TranslationReader:
    def __init__(self, cr, lang=None):
        ...
        self.env = leewise.api.Environment(cr, leewise.SUPERUSER_ID, {})
        self._to_translate = []

    def __iter__(self):
        for module, source, name, res_id, ttype, comments, _record_id, value in self._to_translate:
            yield (module, ttype, name, res_id, source, encode(leewise.tools.ustr(value)), comments)
...
class TranslationModuleReader(TranslationReader):
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._path_list = [(path, True) for path in leewise.addons.__path__]
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'leewise.tools.translate:babel_extract_qweb',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if fnmatch.fnmatch(root, '*/data/*'):
                    for fname in fnmatch.filter(files, '*_dashboard.json'):
                        self._babel_extract_terms(fname, path, root, 'leewise.tools.translate:extract_spreadsheet_terms',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if not recursive:
...
def _get_translation_upgrade_queries(cr, field):
    ...
    """
    Model = leewise.registry(cr.dbname)[field.model_name]
