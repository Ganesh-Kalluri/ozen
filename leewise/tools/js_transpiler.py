"""
This code is what let us use ES6-style modules in leewise.
Classic Leewise modules are composed of a top-level :samp:`leewise.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@leewise-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** leewise-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual leewise one with minimal changes.
...
from leewise.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom leewise modules.
    ...
    legacy_leewise_define = get_aliased_leewise_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_leewise_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_leewise_define:
        content += legacy_leewise_define
    return content
...
def url_to_module_path(url):
    """
    Leewise modules each have a name. (leewise.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_leewise_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the leewise.define call.
    ...
    """
    return f"""leewise.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
LEEWISE_MODULE_RE = re.compile(r"""
    \s*                                       # some starting space
    \/(\*|\/).*\s*                            # // or /*
    @leewise-module                              # @leewise-module
...
def is_leewise_module(content):
    """
    Detect if the file is a native leewise module.
    We look for a comment containing @leewise-module.

    :param content: source code
    :return: is this a leewise module that need transpilation ?
    """
    result = LEEWISE_MODULE_RE.match(content)
    return bool(result)


def get_aliased_leewise_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the leewise source base being:
    ...
    > leewise.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @leewise-module */
        // after
        /** @leewise-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @leewise-module */
        // after
        /** @leewise-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = LEEWISE_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nleewise.define(`%s`, ['%s'], function (require) {
                        return require('%s');
