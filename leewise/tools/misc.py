...
import leewise
import leewise.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from leewise.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
from leewise.tools.float_utils import float_round
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if leewise.tools.config['db_host']:
        env['PGHOST'] = leewise.tools.config['db_host']
    if leewise.tools.config['db_port']:
        env['PGPORT'] = str(leewise.tools.config['db_port'])
    if leewise.tools.config['db_user']:
        env['PGUSER'] = leewise.tools.config['db_user']
    if leewise.tools.config['db_password']:
        env['PGPASSWORD'] = leewise.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',), env=None):
    ...
    addons_paths = leewise.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None, env=None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/leewise/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env):
    ...
    Examples::
        ...
        >>> with leewise.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with leewise.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('leewise.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('leewise.foo.bar'):
            do_suff()
    """
...
def ignore(*exc):
    warnings.warn("Since 16.0 `leewise.tools.ignore` is replaced by `contextlib.suppress`.", DeprecationWarning, stacklevel=2)
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = leewise.fields.Datetime.from_string(value)
            value = leewise.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = leewise.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = leewise.fields.Datetime.from_string(value)
    else:
        timestamp = value
