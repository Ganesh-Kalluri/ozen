    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('leewise.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'leewise.addons'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for Leewise models.

    Leewise models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~leewise.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~leewise.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~leewise.models.AbstractModel`.
    """
    ...
    _module = None              #: the model's module (in the Leewise sense)
    ...
    .. warning::
      ...
      :attr:`~leewise.models.Model._inherits`-ed models, the inherited field will
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], leewise.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], leewise.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from leewise.fields import Char, Integer
    ...
    def _compute_display_name(self):
        ...
        it can be made field-dependent using :attr:`~leewise.api.depends` and
        context-dependent using :attr:`~leewise.api.depends_context`.
        """
        if self._rec_name:
    ...
    def _read_group_fill_temporal(self, data, groupby, annoted_aggregates,
        ...
        if fill_from:
            fill_from = leewise.fields.Datetime.to_datetime(fill_from) if isinstance(fill_from, datetime.datetime) else leewise.fields.Date.to_date(fill_from)
        ...
        if fill_to:
            fill_to = leewise.fields.Datetime.to_datetime(fill_to) if isinstance(fill_to, datetime.datetime) else leewise.fields.Date.to_date(fill_to)
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~leewise.addons.base.models.res_company`).

        For :class:`~leewise.addons.base.models.res_users` relational fields,
    ...
    def unlink(self):
        ...
        from leewise.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~leewise.fields.Integer`,
          :class:`~leewise.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~leewise.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~leewise.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~leewise.fields.Many2one`, the value should be the
          database identifier of the record to set
        * The expected value of a :class:`~leewise.fields.One2many` or
          :class:`~leewise.fields.Many2many` relational field is a list of
          :class:`~leewise.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~leewise.fields.Command.create`,
          :meth:`~leewise.fields.Command.update`,
          :meth:`~leewise.fields.Command.delete`,
          :meth:`~leewise.fields.Command.unlink`,
          :meth:`~leewise.fields.Command.link`,
          :meth:`~leewise.fields.Command.clear`, and
          :meth:`~leewise.fields.Command.set`.
        * For :class:`~leewise.fields.Date` and `~leewise.fields.Datetime`,
          ...
          .. warning::
            ...
            :const:`leewise.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
            :const:`leewise.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
    ...
    def ensure_one(self):
        ...
        :raise leewise.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~leewise.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~leewise.addons.base.models.res_company` or int
    ...
    def _populate_factories(self):
        ...
        See :mod:`leewise.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Leewise models.

    Leewise models are created by inheriting from this class::
