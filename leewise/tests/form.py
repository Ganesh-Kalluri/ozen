# -*- coding: utf-8 -*-
"""
The module :mod:`leewise.tests.form` provides an implementation of a client form
...
class Form:
    ...
    * properly handle defaults & onchanges around x2many fields.
    ...
    of :class:`~leewise.fields.Many2one` fields, one can assign a recordset::
    ...
    For :class:`~leewise.fields.Many2many` fields, the field itself is a
    :class:`~leewise.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~leewise.fields.One2many` are reified as :class:`~O2MProxy`.

    Because the :class:`~leewise.fields.One2many` only exists through its parent,
    ...
    :param record: empty or singleton recordset. An empty recordset will put
                   ...
                   view's data.
    :type record: leewise.models.Model
    ...
    :type view: int | str | leewise.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~leewise.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~leewise.fields.One2many` record.
...
def convert_read_to_form(values, fields):
    ...
    for fname, value in values.items():
        ...
        if field_info['type'] == 'one2many':
            ...
            value = leewise.fields.Datetime.to_string(value)
        elif field_info['type'] == 'date' and isinstance(value, date):
            value = leewise.fields.Date.to_string(value)
        result[fname] = value
    return result
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return leewise.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return leewise.fields.Date.to_string(value)
    return value
