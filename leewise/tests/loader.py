...
from .suite import LeewiseSuite
from .result import LeewiseTestResult


def get_test_modules(module):
    """ Return a list of module for the addons potentially containing tests to
    feed unittest.TestLoader.loadTestsFromModule() """
    results = _get_tests_modules(importlib.util.find_spec(f'leewise.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"leewise.upgrade.{module}",
        f"leewise.addons.{module}.migrations",
        f"leewise.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return LeewiseSuite(sorted(tests, key=lambda t: t.test_sequence))
