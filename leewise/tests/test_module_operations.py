...
import leewise
from leewise.tools import config, topological_sort, unique
from leewise.netsvc import init_logger
from leewise.tests import standalone_tests
import leewise.tests.loader

_logger = logging.getLogger('leewise.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with leewise.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Leewise modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Leewise modules")
...
def test_cycle(args):
    ...
    with leewise.registry(args.database).cursor() as cr:
        env = leewise.api.Environment(cr, leewise.SUPERUSER_ID, {})
...
def test_standalone(args):
    ...
    registry = leewise.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        leewise.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        leewise.tools.config['addons_path'] = ','.join([args.addons_path, leewise.tools.config['addons_path']])
        if args.data_dir:
            leewise.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'leewise.modules.loading': {'level': 'CRITICAL'},
            'leewise.sql_db': {'level': 'CRITICAL'},
            'leewise.models.unlink': {'level': 'WARNING'},
            'leewise.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
