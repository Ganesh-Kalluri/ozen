...
from leewise.tools import DotDict
from leewise.tools.translate import _, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = leewise.registry(db).check_signaling()
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    result = retrying(partial(leewise.api.call_kw, recs, method, args, kw), env)
...
def execute(db, uid, obj, method, *args, **kw):
    with leewise.registry(db).cursor() as cr:
...
def retrying(func, env):
    ...
    :param leewise.api.Environment env: The environment where the registry
