...
import leewise.sql_db
import leewise.tools
from leewise.sql_db import db_connect
from leewise.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not leewise.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and leewise.tools.config.verify_admin_password(passwd):
        return True
    raise leewise.exceptions.AccessDenied()

# This should be moved to leewise.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            leewise.modules.db.initialize(cr)
            leewise.tools.config['load_language'] = lang
            cr.commit()

        registry = leewise.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = leewise.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = leewise.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = leewise.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = leewise.tools.config['db_template']
    ...
    try:
        db = leewise.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if leewise.tools.config['unaccent']:
                ...
                # But in the case of Leewise, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    leewise.sql_db.close_db(db_original_name)
    db = leewise.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = leewise.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = leewise.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            leewise.modules.neutralize.neutralize_database(cr)

    from_fs = leewise.tools.config.filestore(db_original_name)
    to_fs = leewise.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    leewise.modules.registry.Registry.delete(db_name)
    leewise.sql_db.close_db(db_name)
    ...
    fs = leewise.tools.config.filestore(db_name)
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            ...
            if stream:
                leewise.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                leewise.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = leewise.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = leewise.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    leewise.modules.registry.Registry.delete(old_name)
    leewise.sql_db.close_db(old_name)
    ...
    old_fs = leewise.tools.config.filestore(old_name)
    new_fs = leewise.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    leewise.tools.config.set_admin_password(new_password)
    leewise.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        leewise.tools.config['update']['base'] = True
        leewise.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@leewise.tools.mute_logger('leewise.sql_db')
...
def list_dbs(force=False):
    if not leewise.tools.config['list_db'] and not force:
        raise leewise.exceptions.AccessDenied()

    if not leewise.tools.config['dbfilter'] and leewise.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Leewise will not
        ...
        res = sorted(db.strip() for db in leewise.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [leewise.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Leewise
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if leewise.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        leewise.sql_db.close_db(database_name)
...
def exp_list_lang():
    return leewise.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(leewise.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return leewise.release.version
