...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('leewise.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    if sys.version_info[:2] == (3, 9):
        ...
        warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='leewise')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='leewise.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("LEEWISE_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'leewise.http.rpc.request:INFO',
    'leewise.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['leewise:DEBUG', 'leewise.sql_db:INFO', 'leewise.http.rpc:DEBUG'],
    'debug_rpc': ['leewise:DEBUG', 'leewise.sql_db:INFO', 'leewise.http.rpc.request:DEBUG'],
    'debug': ['leewise:DEBUG', 'leewise.sql_db:INFO'],
    'debug_sql': ['leewise.sql_db:DEBUG'],
    'info': [],
    'runbot': ['leewise:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['leewise:WARNING', 'werkzeug:WARNING'],
    'error': ['leewise:ERROR', 'werkzeug:ERROR'],
    'critical': ['leewise:CRITICAL', 'werkzeug:CRITICAL'],
}
