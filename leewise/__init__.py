...
#----------------------------------------------------------
# leewise must be a namespace package for leewise.addons to become one too
...
import sys
assert sys.version_info > (3, 10), "Outdated python version detected, Leewise requires Python >= 3.10 to run."
...
if len(sys.argv) > 1 and sys.argv[1] == 'gevent':
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in leewise/debian/copyright)
        while 1:
            state = conn.poll()
...
from . import fields
from . import api
from leewise.tools.translate import _, _lt
