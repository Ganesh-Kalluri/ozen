...
import leewise
from leewise.modules.db import FunctionStatus
from leewise.osv.expression import get_unaccent_wrapper
from .. import SUPERUSER_ID
from leewise.sql_db import TestCursor
from leewise.tools import (
    config, existing_tables, lazy_classproperty,
    lazy_property, sql, Collector, OrderedSet, SQL,
    format_frame
)
from leewise.tools.func import locked
from leewise.tools.lru import LRU

_logger = logging.getLogger(__name__)
_schema = logging.getLogger('leewise.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                leewise.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                leewise.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        self._assertion_report = leewise.tests.result.LeewiseTestResult()
        ...
        self._db = leewise.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = leewise.modules.db.has_unaccent(cr)
            self.has_trigram = leewise.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = leewise.api.Environment(cr, SUPERUSER_ID, context)
