...
import leewise
import leewise.tools as tools
import leewise.release as release
from leewise.tools import pycompat
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `leewise.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^leewise\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("leewise.addons.base.maintenance.migrations", "leewise.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``leewise.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook leewise.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in leewise.addons.__path__:
        leewise.addons.__path__.append(dd)

    # hook leewise.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in leewise.addons.__path__:
            leewise.addons.__path__.append(ad)

    # hook leewise.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in leewise.addons.__path__ and os.path.isdir(base_path):
        leewise.addons.__path__.append(base_path)

    # hook leewise.upgrade on upgrade-path
    from leewise import upgrade
    ...
    # create decrecated module alias from leewise.addons.base.maintenance.migrations to leewise.upgrade
    spec = importlib.machinery.ModuleSpec("leewise.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["leewise.addons.base.maintenance"] = maintenance_pkg
    sys.modules["leewise.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to leewise and "crm"-like to leewise.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in leewise.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = leewise.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(leewise.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'leewise.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in leewise.addons.__path__:
