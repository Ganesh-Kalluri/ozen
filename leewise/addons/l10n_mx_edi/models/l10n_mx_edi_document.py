...
from leewise import _, api, models, modules, fields, tools
from leewise.exceptions import UserError
from leewise.osv import expression
from leewise.tools import frozendict
from leewise.tools.float_utils import float_is_zero
...
class L10nMxEdiDocument(models.Model):
    ...
    _description = "Mexican documents that needs to transit outside of Leewise"
    ...
    def _is_cfdi_negative_lines_allowed(self):
        ...
        or global discounts. This method allows leewise to distribute the negative discount lines to each others making
        ...
        :return: True if leewise needs to distribute the negative discount lines, False otherwise.
        """
        return True
...
""" % {'boundary': boundary, 'cfdi_b64': cfdi_b64}
    ...
    def _fetch_and_update_sat_status(self, batch_size=100, extra_domain=None):
        ...
        In the second case, the cancellation could be done Leewise-side and then we need to check if the SAT is up-to-date,
        or could be done manually government-side forcing Leewise to update the invoice's state.
        '''
