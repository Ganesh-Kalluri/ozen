...
QUnit.test("addLink: utility function and special entities", function (assert) {
    const testInputs = {
        ...
        "https://leewise.com/%5B%5D": `<a target="_blank" rel="noreferrer noopener" href="https://leewise.com/%5B%5D">https://leewise.com/[]</a>`,
    };
...
QUnit.test("url", async () => {
    ...
    const messageBody = "https://leewise.com?test=~^|`{}[]#";
...
QUnit.test("url with comma at the end", async () => {
    ...
    const { openDiscuss } = await start();
    await openDiscuss(channelId);
    const messageBody = "Go to https://leewise.com, it's great!";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button:enabled", { text: "Send" });
    await contains(".o-mail-Message a", { text: "https://leewise.com" });
...
QUnit.test("url with number in subdomain", async () => {
    ...
    const { openDiscuss } = await start();
    await openDiscuss(channelId);
    const messageBody = "https://www.45017478-master-all.runbot134.leewise.com/web";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button:enabled", { text: "Send" });
    await contains(".o-mail-Message a", {
        text: "https://www.45017478-master-all.runbot134.leewise.com/web",
    });
});
