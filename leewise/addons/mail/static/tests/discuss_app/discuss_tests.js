...
QUnit.test("Posting message should transform links.", async () => {
    ...
    const { openDiscuss } = await start();
    openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", "test https://www.leewise.in/");
    await click(".o-mail-Composer-send:enabled");
    await contains("a[href='https://www.leewise.in/']");
});
...
QUnit.test("no out-of-focus notif on non-needaction message in channel", async (assert) => {
    ...
    const { env } = await start({
        services: {
            presence: makeFakePresenceService({ isLeewiseFocused: () => false }),
        },
    });
...
QUnit.test("out-of-focus notif on needaction message in channel", async (assert) => {
    ...
    await contains(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-ChatWindow", { count: 0 });
    // simulate receiving a new needaction message with leewise out-of-focused
    const currentPartnerId = pyEnv.currentPartnerId;
    pyEnv.withUser(userId, () =>
        env.services.rpc("/mail/message/post", {
            post_data: {
...
QUnit.test("receive new chat message: out of leewise focus (notification, chat)", async (assert) => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Dumbledore" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
    ...
    await contains(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-ChatWindow", { count: 0 });
    // simulate receiving a new message with leewise out-of-focused
    pyEnv.withUser(userId, () =>
        env.services.rpc("/mail/message/post", {
            post_data: {
                body: "New message",
...
QUnit.test("no out-of-focus notification on receiving self messages in chat", async () => {
    ...
    await contains(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-ChatWindow", { count: 0 });
    // simulate receiving a new message of self with leewise out-of-focused
...
QUnit.test("receive new chat messages: out of leewise focus (tab title)", async (assert) => {
    let stepCount = 0;
    const pyEnv = await startServer();
    ...
    // simulate receiving a new message in chat 1 with leewise out-of-focused
    ...
    // simulate receiving a new message in chat 2 with leewise out-of-focused
    ...
    // simulate receiving another new message in chat 2 with leewise focused
    pyEnv["bus.bus"]._sendone(channel_2, "discuss.channel/new_message", {
        id: channelId_2,
        message: {
            id: 128,
    ...
    async () => {
        ...
        pyEnv["discuss.channel"].create([
            ...
            },
            {
                channel_member_ids: [
                    Command.create({ partner_id: pyEnv.currentPartnerId }),
                    Command.create({ partner_id: TEST_USER_IDS.leewisebotId }),
                ],
                channel_type: "chat",
            },
        ]);
        ...
        await click(".o-mail-DiscussSidebarChannel", { text: "LeewiseBot" });
