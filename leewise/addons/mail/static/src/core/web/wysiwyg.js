...
import { closestBlock, setCursorEnd } from "@web_editor/js/editor/leewise-editor/src/LeewiseEditor";
...
patch(Wysiwyg.prototype, {
    ...
    async startEdition() {
        ...
        if (this.inDiscuss) {
            this.leewiseEditor.document.addEventListener("keydown", this.triggerMentionList, true);
            this.leewiseEditor.document.addEventListener("click", this.triggerMentionList, true);
        }
        return res;
    },
    destroy() {
        super.destroy();
        if (this.inDiscuss && this.leewiseEditor) {
            this.leewiseEditor.document.removeEventListener("keydown", this.triggerMentionList, true);
            this.leewiseEditor.document.removeEventListener("click", this.triggerMentionList, true);
        }
    },
    async triggerMentionList(ev) {
        ...
        // Let event be handled by bubbling handlers and other handlers from Leewise Editor first.
        ...
        if (
            ...
            this.stepBeforeMention = this.leewiseEditor._historySteps.length - 2;
            const closest = closestBlock(this.leewiseEditor.document.getSelection().anchorNode);
    ...
    selectMention(ev, option) {
        ...
        this.leewiseEditor.historyRevertUntil(this.stepBeforeMention);
        this.leewiseEditor.execCommand("insert", mentionBlock);
        this.leewiseEditor.execCommand("insert", space);
