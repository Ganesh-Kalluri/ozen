    ...
    def test_01_discuss_channel_access(self):
        ...
        for user_key, channel_key, membership, operation, result in tests:
            if result:
                ...
                try:
                    with self.assertRaises(AccessError), mute_logger("leewise.sql_db"), mute_logger(
                        "leewise.addons.base.models.ir_model"
                    ), mute_logger("leewise.addons.base.models.ir_rule"), mute_logger(
                        "leewise.models.unlink"
                    ):
                        self._execute_action_channel(
                            user_key, channel_key, membership, operation, result
                        )
