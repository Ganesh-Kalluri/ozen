...
(function () {
        ...
        define(name, deps, factory) {
            ...
            if (!this.factories.has(name)) {
                this.factories.set(name, {
                    deps,
                    fn: factory,
                    ignoreMissingDeps: globalThis.__leewiseIgnoreMissingDependencies,
                });
                this.addJob(name);
    ...
    if (!globalThis.leewise) {
        globalThis.leewise = {};
    }
    const leewise = globalThis.leewise;
    if (leewise.debug && !new URLSearchParams(location.search).has("debug")) {
        // remove debug mode if not explicitely set in url
        leewise.debug = "";
    }

    const loader = new ModuleLoader();
    leewise.define = loader.define.bind(loader);

    leewise.loader = loader;
})();
