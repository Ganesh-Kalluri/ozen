...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Leewise" });
    assert.strictEqual(env.services.title.current, "Leewise");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Leewise", chat: null });
    assert.strictEqual(env.services.title.current, "Leewise");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Leewise - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Leewise", action: "Import" });
    assert.strictEqual(env.services.title.current, "Leewise - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Leewise", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Leewise - Import"); // parts is a copy!
});
