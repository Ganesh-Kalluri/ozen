...
from leewise.http import content_disposition, request
from leewise.tools import lazy_property, osutil, pycompat
from leewise.tools.misc import xlsxwriter
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name='',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: leewise.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from Leewise's export data to whatever the
