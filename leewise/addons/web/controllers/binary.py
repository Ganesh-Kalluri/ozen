...
try:
    ...
    from leewise.tools._vendor.send_file import send_file

import leewise
import leewise.modules.registry
from leewise import SUPERUSER_ID, _, http
from leewise.addons.base.models.assetsbundle import ANY_UNIQUE
from leewise.exceptions import AccessError, UserError
from leewise.http import request, Response
from leewise.tools import file_open, file_path, replace_exceptions
from leewise.tools.image import image_guess_size_from_field_name
from leewise.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Leewise is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Leewise is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if leewise.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=leewise.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or leewise.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                registry = leewise.modules.registry.Registry(dbname)
