...
from leewise.modules import get_manifest
from leewise.http import request
from leewise.tools import lazy
...
@lazy
def CONTENT_MAXAGE():
    warnings.warn("CONTENT_MAXAGE is a deprecated alias to leewise.http.STATIC_CACHE_LONG", DeprecationWarning)
...
class WebClient(http.Controller):
    ...
    def bootstrap_translations(self, mods=None):
        ...
        if mods is None:
            mods = leewise.conf.server_wide_modules or []
            if request.db:
    ...
    def translations(self, unique, mods=None, lang=None):
        ...
        if mods:
            ...
            mods = list(request.env.registry._init_modules) + (leewise.conf.server_wide_modules or [])
    ...
    def version_info(self):
        return leewise.service.common.exp_version()
