...
from leewise.http import content_disposition, dispatch_rpc, request, Response
from leewise.service import db
from leewise.tools.misc import file_open, str2bool
from leewise.tools.translate import _

from leewise.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = leewise.tools.config.verify_admin_password('admin')
        d['list_db'] = leewise.tools.config['list_db']
        d['langs'] = leewise.service.db.exp_list_lang()
        d['countries'] = leewise.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = leewise.service.db.list_db_incompatible(d['databases'])
        except leewise.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = leewise.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            leewise.service.db.check_super(master_pwd)
            ...
            dump_stream = leewise.service.db.dump_db(name, None, backup_format)
