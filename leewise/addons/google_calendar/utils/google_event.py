...
from leewise.tools import email_normalize, ReadonlyDict
import logging
...
class GoogleEvent(abc.Set):
    ...
    Inspired by Leewise recordset, one instance can be a single Google event or a
    ...
    def leewise_id(self, env):
        self.leewise_ids(env)  # load ids
        return self._leewise_id

    def _meta_leewise_id(self, dbname):
        """Returns the Leewise id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_leewise_id' % dbname)
        if o_id:
            return int(o_id)

    def leewise_ids(self, env):
        ids = tuple(e._leewise_id for e in self if e._leewise_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_leewise_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_leewise_ids_from_metadata(env, model)

        return tuple(e._leewise_id for e in self)

    def _load_leewise_ids_from_metadata(self, env, model):
        unsure_leewise_ids = tuple(e._meta_leewise_id(env.cr.dbname) for e in self)
        leewise_events = model.browse(_id for _id in unsure_leewise_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Leewise id.
        # Therefore, we only consider Leewise records without google id when trying
        # to match events.
        o_ids = leewise_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            leewise_id = e._meta_leewise_id(env.cr.dbname)
            if leewise_id in o_ids:
                e._events[e.id]['_leewise_id'] = leewise_id

    def _load_leewise_ids_from_db(self, env, model):
        leewise_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in leewise_events}  # {google_id: leewise_id}
        existing_google_ids = leewise_events.mapped('google_id')
        for e in self:
            leewise_id = mapping.get(e.id)
            if leewise_id:
                e._events[e.id]['_leewise_id'] = leewise_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Leewise.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Leewise (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Leewise) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Leewise several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_leewise_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.leewise_ids(env)
        events.leewise_ids(env)

        return self.filter(lambda e: e._leewise_id)
    ...
    def get_leewise_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.leewise_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.leewise_id(self.env)).base_event_id
