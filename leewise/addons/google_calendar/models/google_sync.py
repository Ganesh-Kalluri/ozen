...
from leewise import api, fields, models, registry, _
from leewise.tools import ormcache_context, email_normalize
from leewise.exceptions import UserError
from leewise.osv import expression

from leewise.addons.google_calendar.utils.google_event import GoogleEvent
from leewise.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from leewise.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Leewise database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Leewise.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Leewise DB (_sync_google2leewise).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_leewise2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2leewise(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in Leewise. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Leewise
        :return: synchronized leewise recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        leewise_values = [
            dict(self._leewise_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_leewise = self.with_context(dont_notify=True)._create_from_google(new, leewise_values)
        cancelled = existing.cancelled()
        cancelled_leewise = self.browse(cancelled.leewise_ids(self.env))

        # Check if it is a recurring event that has been rescheduled.
        # We have to check if an event already exists in Leewise.
        # Explanation:
        ...
        if rescheduled_events:
            ...
            cancelled_leewise += self.env['calendar.event'].search([('google_id', 'in', google_ids_to_remove)])

        cancelled_leewise._cancel()
        synced_records = new_leewise + cancelled_leewise
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and leewise server time are different
            updated = parse(gevent.updated)
            leewise_record = self.browse(gevent.leewise_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not leewise_record.write_date or updated >= pytz.utc.localize(leewise_record.write_date):
                vals = dict(self._leewise_values(gevent, default_reminders), need_sync=False)
                leewise_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= leewise_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Leewise to Google
    ...
    @api.model
    def _leewise_values(self, google_event: GoogleEvent, default_reminders=()):
