...
from leewise import api, fields, models, Command
from leewise.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from leewise.addons.google_calendar.models.google_sync import google_calendar_token
from leewise.loglevels import exception_to_unicode
from leewise.tools import str2bool
...
class User(models.Model):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = self.env['calendar.recurrence']._sync_google2leewise(recurrences)
        synced_events = self.env['calendar.event']._sync_google2leewise(events - recurrences, default_reminders=default_reminders)

        # Leewise -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_leewise2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_leewise2google(calendar_service)

        return bool(events | synced_events) or bool(recurrences | synced_recurrences)

    def _sync_single_event(self, calendar_service: GoogleCalendarService, leewise_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2leewise(event, default_reminders=default_reminders)
        # Leewise -> Google
        leewise_event.with_context(send_updates=send_updates)._sync_leewise2google(calendar_service)
        return bool(leewise_event | synced_events)
