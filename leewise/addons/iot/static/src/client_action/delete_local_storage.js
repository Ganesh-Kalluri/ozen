...
class MainComponent extends Component {
	setup() {
        const links = JSON.parse(browser.localStorage.getItem("leewise-iot-linked_reports"));
	...
	removeFromLocal(id) {
        ...
        if (Object.keys(links).length == 0)
            ...
            browser.localStorage.removeItem("leewise-iot-linked_reports");
        else
            // Replace the entry in LocalStorage by the same object with the key 'id' removed
            browser.localStorage.setItem("leewise-iot-linked_reports", JSON.stringify(links))
