...
export class IoTRestartLeewiseOrReboot extends Component {
    setup() {
        super.setup();
    ...
    async onClick() {
        this.dialog.add(ConfirmationDialog, {
            body:
                this.props.action == "restart_leewise"
                    ? _t("Are you sure you want to restart Leewise on the IoT box?")
                    : _t("Are you sure you want to reboot the IoT box?"),
            confirm: () => this.restartLeewiseOrReboot(),
            cancel: () => {},
        });
    }
    ...
    async callRestartMethodOnServer() {
        /// Call "restart_leewise_or_reboot" method from "hw_posbox_homepage" controller
        let restartResponse;
        try {
            ...
            restartResponse = await this.rpc(this.ip_url + "/iot_restart_leewise_or_reboot", {
...
IoTRestartLeewiseOrReboot.template = `iot.IoTRestartLeewiseOrReboot`;

IoTRestartLeewiseOrReboot.props = {
    ...standardWidgetProps,
    btn_name: { type: String },
    action: { type: String },
};

export const ioTRestartLeewiseOrReboot = {
    component: IoTRestartLeewiseOrReboot,
    extractProps: ({ attrs }) => {
        return {
...
registry.category("view_widgets").add("iot_restart_leewise_or_reboot", ioTRestartLeewiseOrReboot);
