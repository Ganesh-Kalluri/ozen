...
class TestIncomingTransactionResponse(BaseCase):
    @patch.dict(
        "sys.modules", {
            ...
            "leewise.addons.hw_drivers": MagicMock(),
            # Mock the modules IngenicoDriver imports so the imports don't fail
            "leewise.addons.hw_drivers.driver": MagicMock(),
            "leewise.addons.hw_drivers.event_manager": MagicMock(),
            "leewise.addons.hw_drivers.iot_handlers.interfaces.SocketInterface": MagicMock(),
        }
    )
    def setUp(self):
        # pylint: disable=import-outside-toplevel
        from leewise.addons.iot.iot_handlers.drivers.IngenicoDriver import IncomingIngenicoMessage
...
class TestOutgoingIngenicoMessage(BaseCase):
    ...
    def setUp(self):
        # pylint: disable=import-outside-toplevel
        from leewise.addons.iot.iot_handlers.drivers.IngenicoDriver import OutgoingIngenicoMessage
