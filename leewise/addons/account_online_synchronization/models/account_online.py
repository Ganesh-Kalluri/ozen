...
from leewise import api, fields, models, modules, tools, _
from leewise.exceptions import UserError, CacheMiss, MissingError, ValidationError, RedirectWarning
from leewise.http import request
from leewise.addons.account_online_synchronization.models.leewisefin_auth import LeewiseFinAuth
from leewise.tools.misc import format_amount, format_date, get_lang

_logger = logging.getLogger(__name__)
pattern = re.compile("^[a-z0-9-_]+$")
runbot_pattern = re.compile(r"^https:\/\/[a-z0-9-_]+\.[a-z0-9-_]+\.leewise\.com$")

class LeewiseFinRedirectException(UserError):
...
class AccountOnlineAccount(models.Model):
    ...
    fetching_status = fields.Selection(
        selection=[
            ...
            ('processing', 'Processing'),  # When currently importing in leewise
            ('done', 'Done'),  # When every transaction have been imported in leewise
        ]
    )
    ...
    def _refresh(self):
        """
            ...
            - Provider is in the process of importing the transactions so we should wait until he has
                finished before fetching them in Leewise
        ...
        while True:
            ...
            resp_json = self.account_online_link_id._fetch_leewise_fin('/proxy/v1/refresh', data=data)
    ...
    def _retrieve_transactions(self, date=None, include_pendings=False):
        ...
        while True:
            ...
            resp_json = self.account_online_link_id._fetch_leewise_fin('/proxy/v1/transactions', data=data)
    ...
    def _get_filtered_transactions(self, new_transactions):
        ...
        # Remove transactions already imported in Leewise
...
class AccountOnlineLink(models.Model):
    ...
    has_unlinked_accounts = fields.Boolean(default=True, help="True if that connection still has accounts that are not linked to an Leewise journal")

    # Information received from LeewiseFin, should not be tampered with
    ...
    def _handle_leewisefin_redirect_exception(self, mode='link'):
        if mode == 'link':
    ...
    def _fetch_leewise_fin(self, url, data=None, ignore_status=False):
        """
        Method used to fetch data from the Leewise Fin proxy.
        ...
        endpoint_url = 'https://%s.leewisefin.com%s' % (proxy_mode, url)
        ...
        data['utils'] = {
            ...
            'server_version': leewise.release.serie,
            'db_uuid': self.env['ir.config_parameter'].sudo().get_param('database.uuid'),
            'cron': cron,
        }
        if request:
            # many banking institutions require the end-user IP/user_agent for traceability
            # of client-initiated actions. It won't be stored on leewisefin side.
            data['utils']['psu_info'] = {
        ...
        try:
            ...
            resp = requests.post(url=endpoint_url, json=data, timeout=timeout, auth=LeewiseFinAuth(record=self.sudo()))
            resp_json = resp.json()
    ...
    def _handle_response(self, resp_json, url, data, ignore_status=False):
        ...
        if not resp_json.get('error'):
            ...
            state = result.get('leewisefin_state') or False
            ...
            if error.get('code') == 101:  # access token expired, not an error
                ...
                return self._fetch_leewise_fin(url, data, ignore_status)
                ...
                raise LeewiseFinRedirectException(mode=error.get('data', {}).get('mode', 'link'))
            ...
            state = error_details.get('leewisefin_state') or 'error'
    ...
    def _log_information(self, state, subject=None, message=None, reset_tx=False):
        ...
        try:
            if reset_tx:
                ...
                leewise_help_description = f'''ClientID: {self.client_id}\nInstitution: {self.name}\nError Reference: {error_reference}\nError Message: {message}\n'''
                leewise_help_summary = f'Bank sync error ref: {error_reference} - Provider: {provider} - Client ID: {self.client_id}'
                url_params = urllib.parse.urlencode({'stage': 'bank_sync', 'summary': leewise_help_summary, 'description': leewise_help_description[:1500]})
                url = f'https://www.leewise.in/help?{url_params}'
            ...
            if subject and message:
                ...
                if reset_tx:
                    message_post = Markup('%s<br>%s <a href="%s" >%s</a>') % (message_post, _("You can contact Leewise support"), url, _("Here"))
            ...
            if reset_tx:
                ...
                # and then raise a redirectWarning error so that customer can easily open an issue with Leewise.
                self.env.cr.commit()
                action_id = {
    ...
    def _get_access_token(self):
        for link in self:
            resp_json = link._fetch_leewise_fin('/proxy/v1/get_access_token', ignore_status=True)
    ...
    def _get_refresh_token(self):
        ...
        for link in self.sudo():
            resp_json = link._fetch_leewise_fin('/proxy/v1/renew_token', ignore_status=True)
    ...
    def unlink(self):
        ...
        for link in self:
            try:
                resp_json = link.with_context(delete_sync=True)._fetch_leewise_fin('/proxy/v1/delete_user', data={'provider_data': link.provider_data}, ignore_status=True)  # delete proxy user
                if resp_json.get('delete', True) is True:
                    to_unlink += link
            except LeewiseFinRedirectException:
    ...
    def _fetch_accounts(self, online_identifier=False):
        ...
        while True:
            ...
            resp_json = self._fetch_leewise_fin('/proxy/v1/accounts', data)
        ...
        try:
            ...
            for online_account in accounts_to_synchronize:
                ...
                try:
                    ...
                    raise
                except LeewiseFinRedirectException as redirect_exception:
                    self._notify_connection_update(
                        journal=journal,
                        connection_state_details={
                            'status': 'error',
                            'error_type': 'leewisefin_redirect',
                            'action': self._handle_leewisefin_redirect_exception(mode=redirect_exception.mode),
                        },
                    )
                    raise
            ...
            return
        except LeewiseFinRedirectException as e:
            return self._handle_leewisefin_redirect_exception(mode=e.mode)

    def _get_consent_expiring_date(self):
        self.ensure_one()
        resp_json = self._fetch_leewise_fin('/proxy/v1/consent_expiring_date', ignore_status=True)
    ...
    def _authorize_access(self, data_access_token):
        ...
        try:
            resp_json = self._fetch_leewise_fin('/proxy/v1/authorize_access', data)
    ...
    def success(self, mode, data):
        ...
        try:
            ...
            message = _("This version of Leewise appears to be outdated and does not support the '%s' sync mode. "
    ...
    def exchange_token(self, exchange_token):
        ...
        resp_json = self._fetch_leewise_fin('/proxy/v1/exchange_token', data=data, ignore_status=True)
        ...
        action = {
            ...
            'params': {
                ...
                'includeParam': {
                    ...
                    'serverVersion': leewise.release.serie,
                }
            },
        }
