...
class LeewiseFinAuth(requests.auth.AuthBase):
    """ This is used to sign the request going towards LeewiseFin
        e.g.:
            requests.get(LEEWISEFIN + '/example', auth=LeewiseFinAuth())
            By using `auth=LeewiseFinAuth(self)` when doing a http request, the request will be signed and
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'leewisefin-client-id': self.client_id,
            'leewisefin-access-token': self.access_token,
            'leewisefin-signature': base64.b64encode(h.digest()),
            'leewisefin-timestamp': msg_timestamp,
        })
        return request
