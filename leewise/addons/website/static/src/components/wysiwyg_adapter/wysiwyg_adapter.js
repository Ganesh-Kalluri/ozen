...
import { isMediaElement } from '@web_editor/js/editor/leewise-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@leewise/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.leewiseEditor.options.filterMutationRecords;
        Object.assign(this.leewiseEditor.options, {
            /**
             * @override
             */
        ...
        // Disable LeewiseEditor observer's while setting up classes
        this.leewiseEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with leewiseEditor
        // observer) as the widgets might trigger DOM mutations.
        this._setObserver();
        this.leewiseEditor.observerActive();
    }
    /**
    ...
    async destroy() {
        ...
        const mod = await leewise.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await leewise.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.leewiseEditor.filterMutationRecords(records);
        ...
        this.leewiseEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.leewiseEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by LeewiseEditor when text is entered.
    ...
    async _createSnippetsMenuInstance(options = {}) {
        const snippetsEditor = await leewise.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _toggleMegaMenu(toggleEl) {
        ...
        this.leewiseEditor.observerUnactive("toggleMegaMenu");
        megaMenuEl.classList.add('o_no_parent_editor');
        this.leewiseEditor.observerActive("toggleMegaMenu");
