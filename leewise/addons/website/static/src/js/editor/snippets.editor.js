...
import * as LeewiseEditorLib from "@web_editor/js/editor/leewise-editor/src/utils/utils";
import { Component, onMounted, useRef, useState } from "@leewise/owl";
...
const getTraversedNodes = LeewiseEditorLib.getTraversedNodes;
...
const wSnippetMenu = weSnippetEditor.SnippetsMenu.extend({
    ...
    _getDragAndDropOptions(options = {}) {
        // TODO: This is currently not in use by Leewise's D&D
        // There is currently no way in Leewise D&D to offset the edge scrolling.
     ...
     */
    _getSelection() {
        return this.options.wysiwyg.leewiseEditor.document.getSelection();
    },
    /**
     * @override
     */
    _addToolbar() {
        ...
        this.options.wysiwyg.leewiseEditor.computeFontSizeSelectorValues();
    },
    /**
    ...
    _handleTextOptions(targetEl, optionClassList, applyTextOption = () => {}) {
        ...
        if (selectedTextEl) {
            const restoreCursor = LeewiseEditorLib.preserveCursor(this.$body[0].ownerDocument);
            ...
            this.options.wysiwyg.leewiseEditor.historyResetLatestComputedSelection();
