 ...
 * corner to provide the authenticated user an access to his leewise backend.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!window.frameElement) {
        ...
        // - Having an event that's fire only when the page is from Leewise avoids
        //   weird behaviours. (e.g. if we want to clear out the iframe, it might
        //   fire an DOMContentLoaded on a non leewise page)
        window.frameElement.dispatchEvent(new CustomEvent('LeewiseFrameContentLoaded'));
    }
});
