...
import "@website/libs/zoomleewise/zoomleewise";
...
export const WebsiteRoot = publicRootData.PublicRoot.extend({
    ...
    start: function () {
        ...
        this.$('.zoomable img[data-zoom]').zoomLeewise();
    ...
    async _loadGMapAPI(editableMode, refetch) {
        ...
        if (refetch || !this._gmapAPILoading) {
            this._gmapAPILoading = new Promise(async resolve => {
                ...
                window.leewise_gmap_api_post_load = (async function leewise_gmap_api_post_load() {
                ...
                await loadJS(`https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=leewise_gmap_api_post_load&key=${encodeURIComponent(key)}`);
            });
        }
