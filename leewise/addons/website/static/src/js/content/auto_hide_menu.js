...
async function autoHideMenu(el, options) {
    ...
    function _adapt() {
        ...
        const leewiseEditor = wysiwyg && wysiwyg.leewiseEditor;
        if (leewiseEditor) {
            leewiseEditor.observerUnactive("adapt");
            leewiseEditor.withoutRollback(__adapt);
            leewiseEditor.observerActive("adapt");
            return;
        }
        __adapt();
    }
