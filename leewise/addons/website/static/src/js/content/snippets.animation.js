...
registry.slider = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        if (this.editableMode) {
            ...
            this.$el.on("slide.bs.carousel.slider", () => {
                this.options.wysiwyg.leewiseEditor.observerUnactive();
            });
            this.$el.on("slid.bs.carousel.slider", () => {
                this.options.wysiwyg.leewiseEditor.observerActive();
            });
        }
        return this._super.apply(this, arguments);
    },
    ...
    _computeHeights: function () {
        ...
        $items.toArray().forEach((el) => {
            ...
            this.options.wysiwyg && this.options.wysiwyg.leewiseEditor.observerUnactive('_computeHeights');
            $item.addClass('active');
            this.options.wysiwyg && this.options.wysiwyg.leewiseEditor.observerActive('_computeHeights');
...
registry.Parallax = Animation.extend({
    ...
    _updateBgCss(cssValues) {
        ...
        if (this.options.wysiwyg) {
            this.options.wysiwyg.leewiseEditor.observerUnactive('_updateBgCss');
        }
        this.$bg.css(cssValues);
        if (this.options.wysiwyg) {
            this.options.wysiwyg.leewiseEditor.observerActive('_updateBgCss');
        }
    },
...
registry.mediaVideo = publicWidget.Widget.extend(MobileYoutubeAutoplayMixin, {
    ...
    start: function () {
        ...
        // integrated with Leewise (this refactoring should be done in master).

        const proms = [this._super.apply(this, arguments)];
        let iframeEl = this.el.querySelector(':scope > iframe');

        // The following code is only there to ensure compatibility with
        // videos added before bug fixes or new Leewise versions where the
 ...
 * shapes in future Leewise versions.
 *
 * /!\
...
￼    */
    _setImgSrc(svg, resolve) {
        ...
        preloadedImg.onload = () => {
            ...
            this.options.wysiwyg && this.options.wysiwyg.leewiseEditor.observerUnactive("setImgHoverEffectSrc");
            this.el.src = preloadedImg.getAttribute('src');
            this.options.wysiwyg && this.options.wysiwyg.leewiseEditor.observerActive("setImgHoverEffectSrc");
...
registry.TextHighlight = publicWidget.Widget.extend({
    ...
    async start() {
        ...
        this._adaptOnResize = throttleForAnimation(() => {
            this.options.wysiwyg?.leewiseEditor.observerUnactive("textHighlightResize");
            for (const textEl of this.el.querySelectorAll(".o_text_highlight")) {
                // Remove old effect, normalize content, redraw SVGs...
                switchTextHighlight(textEl, getCurrentTextHighlight(textEl));
            }
            this.options.wysiwyg?.leewiseEditor.observerActive("textHighlightResize");
        });
        if (!this.editableMode) {
