...
export class WebsitePreview extends Component {
    setup() {
        ...
        onMounted(() => {
            ...
            // LeewiseFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('LeewiseFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        });
        ...
        /**
         * This removes the 'Leewise' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('LeewiseFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('LeewiseFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.websiteService.websiteRootInstance = undefined;
            this.iframe.el.addEventListener('LeewiseFrameContentLoaded', resolve, { once: true });
            if (url) {
