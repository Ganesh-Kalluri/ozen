...
import leewise
from leewise.tests.common import HttpCase, HOST
from leewise.tools.misc import hmac, DotDict, frozendict
...
def MockRequest(
    ...
    request = Mock(
        ...
        type='http',
        future_response=leewise.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            leewise.http.get_default_session(),
            geoip={'country_code': country_code},
            sale_order_id=sale_order_id,
            website_sale_current_pl=website_sale_current_pl,
        ),
        geoip=leewise.http.GeoIP('127.0.0.1'),
    ...
    #
    #   router = leewise.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        leewise.http._request_stack.push(request)
        s.callback(leewise.http._request_stack.pop)
        s.enter_context(patch('leewise.http.root.get_db_router', router))
