...
class TestWebsiteAssets(leewise.tests.HttpCase):
...
class TestWebAssets(leewise.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with leewise.tools.mute_logger('leewise.addons.web.controllers.binary'):
            self.assertEqual(
