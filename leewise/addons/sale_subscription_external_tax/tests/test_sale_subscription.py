...
from leewise.tests.common import tagged
...
class TestSaleSubscriptionExternalCommon:
    ...
    def patch_set_external_taxes(self):
        ...
        with patch('leewise.addons.account_external_tax.models.account_move.AccountMove._set_external_taxes', autospec=True) as mocked_set, \
             patch('leewise.addons.account_external_tax.models.account_move.AccountMove._post', lambda self, *args, **kwargs: self), \
             patch('leewise.addons.account_external_tax.models.account_external_tax_mixin.AccountExternalTaxMixin._compute_is_tax_computed_externally', is_computed_externally):
            yield mocked_set

    @contextmanager
    def patch_set_external_taxes_so(self, new_sale_set_external_taxes):
        with patch('leewise.addons.sale_external_tax.models.sale_order.SaleOrder._set_external_taxes', new_sale_set_external_taxes):
            yield
