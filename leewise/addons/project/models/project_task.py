...
from leewise import api, Command, fields, models, tools, SUPERUSER_ID, _, _lt
from leewise.addons.rating.models import rating_data
from leewise.addons.web_editor.tools import handle_history_divergence
from leewise.exceptions import UserError, ValidationError, AccessError
from leewise.osv import expression
from leewise.tools.misc import get_lang
...
class Task(models.Model):
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Leewise-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Leewise-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Leewise-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
