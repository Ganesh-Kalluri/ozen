...
QUnit.module("web_enterprise", function ({ beforeEach }) {
    ...
    QUnit.test("Expiration Panel one app installed, buy subscription", async function (assert) {
        ...
        assert.strictEqual(
            browser.location,
            "https://www.leewise.in/leewise-enterprise/upgrade?num_users=7"
        );
    });
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.strictEqual(
                target.querySelector(".oe_instance_register").innerText,
                "Something went wrong while registering your database. You can try again or contact Leewise Support."
            );
            assert.hasClass(
                target.querySelector(".database_expiration_panel"),
                "alert-danger",
...
Send an email to the subscription owner to confirm the change, enter a new code or buy a subscription.`
            ...
            assert.strictEqual(
                ...
                formatWhiteSpaces(
                    `Your subscription is already linked to a database.
                Send an email to the subscription owner to confirm the change, enter a new code or buy a subscription.
                Unable to send the instructions by email, please contact the Leewise Support
    ...
    QUnit.test("One app installed, upgrade database", async function (assert) {
        ...
        assert.strictEqual(
            browser.location,
            "https://www.leewise.in/leewise-enterprise/upsell?num_users=13&contract=ABC"
        );
    });
    ...
    QUnit.test("One app installed, navigation to renewal page", async function (assert) {
        ...
        assert.strictEqual(
            browser.location,
            "https://www.leewise.in/leewise-enterprise/renew?contract=ABC"
        );
