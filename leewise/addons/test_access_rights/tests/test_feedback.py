...
import leewise
from leewise import SUPERUSER_ID, Command
...
class TestIRRuleFeedback(Feedback):
    ...
    def debug_mode(self):
        leewise.http._request_stack.push(Mock(db=self.env.cr.dbname, env=self.env, debug=True))
        self.addCleanup(leewise.http._request_stack.pop)
        self.env.flush_all()
...
Note: this might be a multi-company issue. Switching company may help - in Leewise, not in real life!
