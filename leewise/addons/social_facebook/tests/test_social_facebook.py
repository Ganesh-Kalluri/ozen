...
from leewise.addons.social_facebook.models.social_post import SocialPostFacebook
from leewise.addons.social_facebook.models.social_stream import SocialStreamFacebook
from leewise.addons.social_facebook.tests.common import SocialFacebookCommon
...
class SocialFacebookCase(SocialFacebookCommon):
    ...
    def test_format_facebook_message(self):
        ...
        message = "Hi, Social is so cool :) Thanks Leewise"
        tags = [{
            'id': 1337,
            'name': 'Leewise - Social',
            'offset': 32,
        ...
        excepted_message = "Hi, Social is so cool :) Thanks @[1337] Leewise-Social"
        self.assertEqual(
            social_stream._format_facebook_message(message, tags),
            excepted_message)

        message = "Hi, Social is so cool :) Thanks Leewise @[45] thisisafaketag Tag another@[45] faketag"
        tags = [
            {
                'id': 1337,
                'name': 'Leewise - Social',
                ...
                'id': 1338,
                'name': 'Leewise Mate and - Social',
                'offset': 58,
                'length': 3
            }]
        excepted_message = "Hi, Social is so cool :) Thanks @[1337] Leewise-Social @ [45] thisisafaketag @[1338] Leewise-Mate-and-Social another@[45] faketag"
        self.assertEqual(
            social_stream._format_facebook_message(message, tags),
            excepted_message)
