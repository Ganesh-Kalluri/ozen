...
class SocialStreamFacebook(models.Model):
    ...
    def _format_facebook_message(self, message, tags):
        ...
        example:
            "Hello Leewise :)" -> "Hello @[542132] Leewise :)"
            "Hello Leewise Social :)" -> "Hello @[542132] Leewise-Social :)"
            "Hello Leewise - Social :)" -> "Hello @[542132] Leewise-Social :)"

        details:
            "Hello @[42] faketag Leewise - Social :)" -> "Hello @ [42] faketag @[542132] Leewise-Social :)""
            -> With associated tag: {name: "Leewise - Social", offset: 20, length: 13}

            We take the message until the tag offset: "Hello @[42] faketag "
            and remove the fake tag in the process "Hello @ [42] faketag "
            Then we insert the forged tag "@[542132] Leewise-Social"
