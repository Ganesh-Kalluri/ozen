...
from leewise.tools import html2plaintext, plaintext2html
...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        for inv in ar_edi_invoices:
            ...
            # in Leewise for consistency, this way if an error ocurrs later in another invoice we will have the ones
            # correctly validated in AFIP in Leewise (CAE, Result, xml response/request).
    ...
    def _l10n_ar_check_afip_auth_verify_required(self):
        ...
        if still_missing:
            if len(still_missing) > 1:
                raise UserError(_(
                    'We can not post these vendor bills in Leewise because the '
            ...
            raise UserError(_(
                'We can not post this vendor bill in Leewise because the AFIP '
