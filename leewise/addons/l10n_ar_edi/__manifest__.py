...
Be able to create journals in Leewise to create electronic customer invoices and report then to AFIP (via webservices).
...
In the electronic journals if you validate an invoice this one will be validated in both Leewise and AFIP. This validation is
...
sent to AFIP and last number used as support for any possible issues on the sequences synchronization between Leewise and AFIP.

    NOTE: From the Journal's form view we are able to force a sync between the leewise sequences to each of the document types in
...
2. Create Sales journals that will represent each one of your AFIP POS (Available in AFIP Portal) you want to use in Leewise.
