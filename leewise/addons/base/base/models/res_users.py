...
from leewise import api, fields, models, tools, SUPERUSER_ID, _, Command
...
from leewise.http import request, DEFAULT_LANG
from leewise.osv import expression
from leewise.service.db import check_super
from leewise.tools import is_html_empty, partition, collections, frozendict, lazy_property
...
class Users(models.Model):
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Leewise (updates, module installation, ...)'))
        self.env.registry.clear_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: leewise.exceptions.AccessDenied when old password is wrong
        :raise: leewise.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this Leewise portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Leewise is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Leewise in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.leewise.in/documentation/17.0/administration/install/deploy.html#https for details.",
                    source
                )
