...
@leewise.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(leewise.tests.TransactionCase):
...
class TestReportsRenderingCommon(leewise.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        leewise.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@leewise.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@leewise.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
