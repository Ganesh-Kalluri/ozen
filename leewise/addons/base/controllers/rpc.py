...
import leewise
from leewise.http import Controller, route, dispatch_rpc, request, Response
from leewise.fields import Date, Datetime, Command
from leewise.tools import lazy, ustr
from leewise.tools.misc import frozendict
...
# User code must use the exceptions defined in ``leewise.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, leewise.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, leewise.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, leewise.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, leewise.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, leewise.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(leewise.tools.exception_to_unicode(e), formatted_info)

    return xmlrpc.client.dumps(fault, allow_none=None, encoding=None)


class LeewiseMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, leewise is sending binary as base64 string.
...
xmlrpc.client.Marshaller = LeewiseMarshaller
