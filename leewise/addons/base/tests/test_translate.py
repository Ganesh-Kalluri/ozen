...
from leewise.tools import sql
from leewise.tools.translate import quote, unquote, xml_translate, html_translate, TranslationImporter, TranslationModuleReader
from leewise.tests.common import TransactionCase, BaseCase, new_test_user, tagged

_stats_logger = logging.getLogger('leewise.tests.stats')
...
class TestLanguageInstall(TransactionCase):
    def test_language_install(self):
        ...
        with patch('leewise.addons.base.models.ir_module.Module._load_module_terms', _load_module_terms):
            wizard.lang_install()
...
class TestTranslationExport(TransactionCase):
    ...
    def test_export_translatable_resources(self):
        ...
        with self.assertNoLogs('leewise.tools.translate', "ERROR"):
...
class TestTranslation(TransactionCase):
    ...
    #     country_1 = Country.create({'name': 'Leewise'})
    #     country_1.with_context(lang='fr_FR').name = 'Leewise_Fr'
    #     country_1.flush_recordset()
    #
    #     country_2 = Country.create({'name': 'Leewise2'})
    #     with self.assertRaises(IntegrityError), mute_logger('leewise.sql_db'):
    #         country_2.name = 'Leewise'
    #         country_2.flush_recordset()
    #
    #     with self.assertRaises(IntegrityError), mute_logger('leewise.sql_db'):
    #         country_3 = Country.create({'name': 'Leewise'})
