...
from leewise.exceptions import AccessDenied, AccessError
from leewise.http import _request_stack

import leewise
import leewise.tools
from leewise.tests import common
from leewise.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def test_xmlrpc_defaultdict_marshalling(self):
        ...
        self.patch(self.registry['res.users'], 'context_get',
                   leewise.api.model(lambda *_: collections.defaultdict(int)))
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, leewise.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
