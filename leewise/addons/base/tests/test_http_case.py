...
from leewise.tests.common import HttpCase, tagged, ChromeBrowser
from leewise.tools import config, logging
...
class TestHttpCase(HttpCase):
    ...
    def test_console_error_string(self):
        with self.assertLogs(level='ERROR') as log_catcher:
            with self.assertRaises(AssertionError) as error_catcher:
                ...
                with patch('leewise.tests.common.ChromeBrowser.take_screenshot', return_value=None):
    ...
    def test_console_log_object(self):
        logger = logging.getLogger('leewise')
        level = logger.level
...
class TestChromeBrowser(HttpCase):
    def setUp(self):
        ...
        with patch.dict('leewise.tools.config.options', {'screencasts': screencasts_dir, 'screenshots': config['screenshots']}):
