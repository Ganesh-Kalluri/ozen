...
function insertList(model, id, anchor = [0, 0]) {
    ...
    return model.dispatch("INSERT_LEEWISE_LIST", {
...
QUnit.test("Can undo a command before a INSERT_LEEWISE_LIST", async (assert) => {
    assert.expect(1);
...
QUnit.test("Rename and remove a list concurrently", async (assert) => {
    insertList(alice, "1");
    await network.concurrent(() => {
        alice.dispatch("RENAME_LEEWISE_LIST", {
            listId: "1",
            name: "test",
        });
        bob.dispatch("REMOVE_LEEWISE_LIST", {
            listId: "1",
        });
    });
    assert.spreadsheetIsSynchronized(
...
QUnit.test("Re-insert and remove a list concurrently", async (assert) => {
    ...
    await network.concurrent(() => {
        ...
        alice.dispatch("RE_INSERT_LEEWISE_LIST", {
            id: "1",
            col: 0,
            row: 0,
