...
QUnit.test("Chart link to leewise menu collaborative", async (assert) => {
    const chartId = "1";
    const sheetId = alice.getters.getActiveSheetId();
    createBasicChart(alice, chartId);
    await network.concurrent(() => {
        alice.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
        bob.dispatch("LINK_LEEWISE_MENU_TO_CHART", {
            chartId,
            leewiseMenuId: "leewiseTestMenu",
        });
    });
    ...
    assert.spreadsheetIsSynchronized(
        [alice, bob, charlie],
        (user) => user.getters.getChartLeewiseMenu(chartId),
        undefined
    );
});
