...
QUnit.module("spreadsheet > pivot_autofill", {}, () => {
    ...
    QUnit.test("Autofill with pivot positions", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "C3", `=LEEWISE.PIVOT(1,"probability","#bar",1,"#foo",1)`);
        assert.strictEqual(
            getPivotAutofillValue(model, "C3", { direction: "left", steps: 1 }),
            `=LEEWISE.PIVOT(1,"probability","#bar",1,"#foo",0)`
        );
        /** Would be negative => just copy the value */
        assert.strictEqual(
            getPivotAutofillValue(model, "C3", { direction: "left", steps: 2 }),
            `=LEEWISE.PIVOT(1,"probability","#bar",1,"#foo",1)`
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "C3", { direction: "right", steps: 1 }),
            `=LEEWISE.PIVOT(1,"probability","#bar",1,"#foo",2)`
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "C3", { direction: "right", steps: 10 }),
            `=LEEWISE.PIVOT(1,"probability","#bar",1,"#foo",11)`
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "C3", { direction: "top", steps: 1 }),
            `=LEEWISE.PIVOT(1,"probability","#bar",0,"#foo",1)`
        );
        /** Would be negative => just copy the value */
        assert.strictEqual(
        ...
        assert.strictEqual(
            getPivotAutofillValue(model, "C3", { direction: "bottom", steps: 1 }),
            `=LEEWISE.PIVOT(1,"probability","#bar",2,"#foo",1)`
        );
        assert.strictEqual(
            ...
            `=LEEWISE.PIVOT(1,"probability","#bar",11,"#foo",1)`
        );
    });
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithPivot();
            setCellContent(model, "A1", `=LEEWISE.PIVOT(1,"probability","bar",B2,"foo",$C$3)`);
            selectCell(model, "A1");
            ...
            assert.equal(
                ...
                `=LEEWISE.PIVOT(1,"probability","bar",B3,"foo",$C$3)`
            );
        }
    );
    ...
    QUnit.test("Can autofill positional col headers horizontally", async (assert) => {
        ...
        setCellContent(model, "B1", `=LEEWISE.PIVOT.HEADER(1,"#product_id",1)`);
        assert.strictEqual(
            getPivotAutofillValue(model, "B1", { direction: "right", steps: 1 }),
            `=LEEWISE.PIVOT.HEADER(1,"#product_id",2)`
        );
        selectCell(model, "B1");
    ...
    QUnit.test("Can autofill positional row headers vertically", async (assert) => {
        ...
        setCellContent(model, "A3", `=LEEWISE.PIVOT.HEADER(1,"date:month","04/2016","#product_id",1)`);
        assert.strictEqual(
            getPivotAutofillValue(model, "A3", { direction: "bottom", steps: 1 }),
            `=LEEWISE.PIVOT.HEADER(1,"date:month","04/2016","#product_id",2)`
        );
        selectCell(model, "A3");
    ...
    QUnit.test("Can autofill positional col horizontally", async (assert) => {
        ...
        setCellContent(model, "B1", `=LEEWISE.PIVOT(1,"probability","#product_id",1)`);
        assert.strictEqual(
            getPivotAutofillValue(model, "B1", { direction: "right", steps: 1 }),
            `=LEEWISE.PIVOT(1,"probability","#product_id",2)`
        );
        selectCell(model, "B1");
        model.dispatch("AUTOFILL_SELECT", { col: 2, row: 0 });
        const tooltipContent = model.getters.getAutofillTooltip().props.content;
    ...
    QUnit.test("Can autofill positional row vertically", async (assert) => {
        ...
        setCellContent(
            model,
            "A3",
            `=LEEWISE.PIVOT(1,"probability","date:month","04/2016","#product_id",1)`
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "A3", { direction: "bottom", steps: 1 }),
            `=LEEWISE.PIVOT(1,"probability","date:month","04/2016","#product_id",2)`
        );
        selectCell(model, "A3");
        model.dispatch("AUTOFILL_SELECT", { col: 1, row: 3 });
        const tooltipContent = model.getters.getAutofillTooltip().props.content;
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "H10", `=LEEWISE.PIVOT(1,"probability","bar","true")`);
            assert.strictEqual(
    ...
    QUnit.test("Autofill pivot values with date in rows", async function (assert) {
        ...
        assert.strictEqual(
            ...
            '=LEEWISE.PIVOT.HEADER(1,"date:month","01/2017")'
        );
        assert.strictEqual(
    ...
    QUnit.test("Autofill pivot values with date in cols", async function (assert) {
        ...
        assert.strictEqual(
            getCellFormula(model, "B1"),
            '=LEEWISE.PIVOT.HEADER(1,"date:day","04/14/2016")'
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "B1", { direction: "right", steps: 1 }),
            '=LEEWISE.PIVOT.HEADER(1,"date:day","04/15/2016")'
        );
        assert.strictEqual(
            getCellFormula(model, "B2"),
            '=LEEWISE.PIVOT.HEADER(1,"date:day","04/14/2016","measure","probability")'
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "B2", { direction: "right", steps: 1 }),
            '=LEEWISE.PIVOT.HEADER(1,"date:day","04/15/2016","measure","probability")'
        );
        assert.strictEqual(
            getCellFormula(model, "B3"),
            '=LEEWISE.PIVOT(1,"probability","foo",1,"date:day","04/14/2016")'
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "B3", { direction: "right", steps: 1 }),
            '=LEEWISE.PIVOT(1,"probability","foo",1,"date:day","04/15/2016")'
        );
    });
    ...
    QUnit.test("Autofill pivot values with date (week) 2020 has 53 weeks", async function (assert) {
        ...
        setCellContent(model, "A1", '=LEEWISE.PIVOT.HEADER(1,"date:week","52/2020")');
        assert.strictEqual(
            getPivotAutofillValue(model, "A1", { direction: "bottom", steps: 1 }),
            '=LEEWISE.PIVOT.HEADER(1,"date:week","53/2020")'
        );
    ...
    QUnit.test("Autofill empty pivot date value", async function (assert) {
        for (const interval of ["day", "week", "month", "quarter", "year"]) {
            ...
            setCellContent(model, "A1", `=LEEWISE.PIVOT.HEADER(1,"date:${interval}","false")`);
            assert.strictEqual(
                getPivotAutofillValue(model, "A1", { direction: "bottom", steps: 1 }),
                `=LEEWISE.PIVOT.HEADER(1,"date:${interval}","false")`
            );
            assert.deepEqual(model.getters.getTooltipFormula(getCellFormula(model, "A1")), [
                { value: "None" },
            ]);
    ...
    QUnit.test("Autofill pivot values with date (month)", async function (assert) {
        ...
        assert.strictEqual(
            getPivotAutofillValue(model, "A3", { direction: "bottom", steps: 1 }),
            `=LEEWISE.PIVOT.HEADER(1,"date:month","05/2016")`
        );
        assert.deepEqual(model.getters.getTooltipFormula(getCellFormula(model, "A3")), [
            { value: "April 2016" },
        ]);
    ...
    QUnit.test("Autofill pivot values with date (no defined interval)", async function (assert) {
        ...
        assert.strictEqual(
            getPivotAutofillValue(model, "A3", { direction: "bottom", steps: 1 }),
            `=LEEWISE.PIVOT.HEADER(1,"date","05/2016")`
        );
    });
    ...
    QUnit.test("Tooltip of pivot formulas", async function (assert) {
        ...
        assert.deepEqual(model.getters.getTooltipFormula(`=LEEWISE.PIVOT.HEADER("1")`, true), [
            { value: "Total" },
        ]);
        assert.deepEqual(model.getters.getTooltipFormula(`=LEEWISE.PIVOT.HEADER("1")`, false), [
            { value: "Total" },
        ]);
    });
    ...
    QUnit.test("Autofill pivot formula with missing pivotId", async function (assert) {
            ...
            sheets: [
                {
                    ...
                    rowNumber: 2,
                    cells: {
                        A1: { content: '=LEEWISE.PIVOT("1","bar","date","05/2023")' },
                        B1: { content: '=LEEWISE.PIVOT.HEADER("1","date","05/2023")' },
                    },
                },
            ],
        });
        assert.strictEqual(
            getPivotAutofillValue(model, "A1", { direction: "bottom", steps: 1 }),
            '=LEEWISE.PIVOT("1","bar","date","05/2023")'
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "B1", { direction: "bottom", steps: 1 }),
            '=LEEWISE.PIVOT.HEADER("1","date","05/2023")'
        );
