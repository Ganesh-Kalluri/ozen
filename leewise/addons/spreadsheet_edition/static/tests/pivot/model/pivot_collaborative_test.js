...
QUnit.test("Rename a pivot", async (assert) => {
    ...
    alice.dispatch("RENAME_LEEWISE_PIVOT", { pivotId: 1, name: "Test" });
...
QUnit.test("Add a pivot", async (assert) => {
    ...
    const cellFormulas = {
        B1: `=LEEWISE.PIVOT.HEADER(1,"foo",1)`, // header col
        A3: `=LEEWISE.PIVOT.HEADER(1,"bar","false")`, // header row
        B2: `=LEEWISE.PIVOT.HEADER(1,"foo",1,"measure","probability")`, // measure
        B3: `=LEEWISE.PIVOT(1,"probability","bar","false","foo",1)`, // value
        F1: `=LEEWISE.PIVOT.HEADER(1)`, // total header rows
        A5: `=LEEWISE.PIVOT.HEADER(1)`, // total header cols
    };
...
QUnit.test("Add two pivots concurrently", async (assert) => {
    ...
    assert.spreadsheetIsSynchronized(
        ...
        `=LEEWISE.PIVOT.HEADER(1,"foo",1)`
    );
    assert.spreadsheetIsSynchronized(
        [alice, bob, charlie],
        (user) => getCellFormula(user, "B26"),
        `=LEEWISE.PIVOT.HEADER(2,"foo",1)`
    );
...
QUnit.test("Rename and remove a pivot concurrently", async (assert) => {
    await insertPivot(alice);
    await network.concurrent(() => {
        alice.dispatch("RENAME_LEEWISE_PIVOT", {
            pivotId: "1",
            name: "test",
        });
