...
topbarMenuRegistry.addChild("data_sources_data", ["data"], (env) => {
    ...
    const lists_items = env.model.getters.getListIds().map((listId, index) => {
        return {
            ...
            sequence: sequence++,
            execute: (env) => {
                env.model.dispatch("SELECT_LEEWISE_LIST", { listId: listId });
                env.openSidePanel("LIST_PROPERTIES_PANEL", {});
            },
            icon: "o-spreadsheet-Icon.LEEWISE_LIST",
            separator: index === env.model.getters.getListIds().length - 1,
        };
    });
    const charts_items = env.model.getters.getLeewiseChartIds().map((chartId, index) => {
        return {
            id: `item_chart_${chartId}`,
            name: env.model.getters.getLeewiseChartDisplayName(chartId),
            sequence: sequence++,
            execute: (env) => {
            ...
            separator: index === env.model.getters.getLeewiseChartIds().length - 1,
        };
    });
    return pivots_items
        .concat(lists_items)
