...
import { Component, onWillUpdateProps } from "@leewise/owl";
...
export class ListingAllSidePanel extends Component {
    ...
    selectListing(listId) {
        this.env.model.dispatch("SELECT_LEEWISE_LIST", { listId });
    }

    resetListingSelection() {
        this.env.model.dispatch("SELECT_LEEWISE_LIST");
    }

    delete(listId) {
        this.env.askConfirmation(_t("Are you sure you want to delete this list?"), () => {
            this.env.model.dispatch("REMOVE_LEEWISE_LIST", { listId });
            this.props.onCloseSidePanel();
        });
    }
}
