...
import { astToFormula, UIPlugin, tokenize } from "@leewise/o-spreadsheet";
...
export class ListAutofillPlugin extends UIPlugin {
    ...
    getNextListValue(formula, isColumn, increment) {
        ...
        if (functionName === "LEEWISE.LIST.HEADER") {
    ...
    getTooltipListFormula(formula, isColumn) {
        ...
        if (isColumn || functionName === "LEEWISE.LIST.HEADER") {
            const fieldName = functionName === "LEEWISE.LIST" ? evaluatedArgs[2] : evaluatedArgs[1];
    ...
    _getListFunction(listId, position, field) {
        return `=LEEWISE.LIST(${listId},${position},"${field}")`;
    }

    _getListHeaderFunction(listId, field) {
        return `=LEEWISE.LIST.HEADER(${listId},"${field}")`;
    }
}
