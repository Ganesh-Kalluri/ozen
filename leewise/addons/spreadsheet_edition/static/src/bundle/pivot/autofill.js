...
autofillRulesRegistry
    .add("autofill_pivot", {
        condition: (cell) =>
            cell &&
            cell.isFormula &&
            cell.content.match(/=\s*LEEWISE\.PIVOT/) &&
            !containsReferences(cell),
        generateRule: (cell, cells) => {
            const increment = cells.filter(
                (cell) => cell && cell.isFormula && cell.content.match(/=\s*LEEWISE\.PIVOT/)
            ).length;
    ...
    .add("autofill_pivot_position", {
        condition: (cell) =>
            cell && cell.isFormula && cell.content.match(/=.*LEEWISE\.PIVOT.*LEEWISE\.PIVOT\.POSITION/),
...
autofillModifiersRegistry
    ...
    .add("PIVOT_POSITION_UPDATER", {
        ...
        apply: (rule, data, getters, direction) => {
            ...
            const pivotId = formulaString.match(/LEEWISE\.PIVOT\.POSITION\(\s*"(\w+)"\s*,/)[1];
            ...
            const field = fields
                .reverse()
                .find((field) =>
                    new RegExp(`LEEWISE\\.PIVOT\\.POSITION.*${field}.*\\)`).test(formulaString)
                );
            const content = formulaString.replace(
                new RegExp(
                    `(.*LEEWISE\\.PIVOT\\.POSITION\\(\\s*"\\w"\\s*,\\s*"${field}"\\s*,\\s*"?)(\\d+)(.*)`
                ),
