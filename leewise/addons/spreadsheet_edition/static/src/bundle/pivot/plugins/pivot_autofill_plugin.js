...
import { UIPlugin, tokenize } from "@leewise/o-spreadsheet";
...
export class PivotAutofillPlugin extends UIPlugin {
    ...
    getPivotNextAutofillValue(formula, isColumn, increment) {
        ...
        for (let i = evaluatedArgs.length - 1; i > 0; i--) {
            ...
            if (
                ...
                if (functionName === "LEEWISE.PIVOT") {
                    return makePivotFormula("LEEWISE.PIVOT", evaluatedArgs);
                } else if (functionName === "LEEWISE.PIVOT.HEADER") {
                    return makePivotFormula("LEEWISE.PIVOT.HEADER", evaluatedArgs);
                }
                return formula;
            }
        }
    ...
    _autofillPivotValue(pivotId, args, isColumn, increment) {
        ...
        return makePivotFormula("LEEWISE.PIVOT", this._buildArgs(pivotId, measure, rows, cols));
    }
    /**
    ...
    _autofillColFromValue(pivotId, nextIndex, currentElement) {
        ...
        return makePivotFormula("LEEWISE.PIVOT.HEADER", this._buildArgs(pivotId, undefined, [], cols));
    }
    /**
    ...
    _autofillRowFromValue(pivotId, currentElement) {
        ...
        return makePivotFormula("LEEWISE.PIVOT.HEADER", this._buildArgs(pivotId, undefined, rows, []));
    }
    /**
