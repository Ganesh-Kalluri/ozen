...
def parse_version(s):
    # can't use leewise.tools.parse_version because pythonpath is screwed from
    # inside pylint on runbot
    return [s.rjust(3, '0') for s in s.split('.')]

class LeewiseBaseChecker(BaseChecker):
    ...
    msgs = {
        'E8502': (
            ...
            'gettext-variable',
            'See https://www.leewise.in/documentation/17.0/developer/misc/i18n/translations.html#variables'
        )
    }
...
def register(linter):
    linter.register_checker(LeewiseBaseChecker(linter))
