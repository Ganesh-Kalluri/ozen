...
class TestConflictMarkers(lint_case.LintCase):
    ...
    def test_conflict_markers(self):
        """ Test that there are no conflict markers left in Leewise files """

        counter = 0

        leewise_path = os.path.abspath(os.path.dirname(leewise.__file__))
        paths = leewise.addons.__path__ + [leewise_path]
        paths.remove(os.path.join(leewise_path, 'addons'))  # avoid checking leewise/addons twice
