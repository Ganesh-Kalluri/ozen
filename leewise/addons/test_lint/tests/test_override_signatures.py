...
import leewise
from leewise.tests.common import get_db_name, tagged
...
methods_to_sanitize = {
    method_name
    for method_name in dir(leewise.models.BaseModel)
...
def get_leewise_module_name(python_module_name):
    if python_module_name.startswith('leewise.addons.'):
        return python_module_name.split('.')[2]
    if python_module_name == 'leewise.models':
        return 'leewise'
...
class TestLintOverrideSignatures(LintCase):
    def test_lint_override_signature(self):
        ...
        registry = leewise.registry(get_db_name())
        for model_name, model_cls in registry.items():
            for method_name, _ in inspect.getmembers(model_cls, inspect.isroutine):
                ...
                parent_module = get_leewise_module_name(parent_class.__module__)
                ...
                for child_class in reverse_mro:
                    ...
                    child_module = get_leewise_module_name(child_class.__module__)
        ...
        #with open('/tmp/leewise-override-signatures.md', 'w') as f:
