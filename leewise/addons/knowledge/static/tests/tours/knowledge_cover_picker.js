...
registry.category("web_tour.tours").add('knowledge_cover_selector_tour', {
    ...
    // Change the search query to find leewise_logo file
    trigger: '.modal-body input.o_we_search',
    extra_trigger: '.modal-body .o_nocontent_help',
    run: 'text leewise_logo',
}, {
    // Choose the leewise_logo cover
    trigger: '.o_existing_attachment_cell img[title*="leewise_logo"]',
    run: 'click',
}, {
    ...
    extra_trigger: '.o_article_active:contains("leewise")',
}, {
    ...
    trigger: '.o_knowledge_sidebar .o_article_name:contains("leewise")',
    extra_trigger: '.o_knowledge_body:not(:has(.o_widget_knowledge_cover))',
}, {
    // Make the add cover button visible
    trigger: '.o_article_active:contains("leewise")',
    run() {},
}, {
    // Click on add cover button
    trigger: '.o_knowledge_add_cover',
    ...
    // Check that leewise logo previously uploaded is shown in the selector as the
    // search query, which is the article name, is "leewise" which is also in the
    // cover attachment's name, and that clicking on it sets it as cover of the
    // current article
    trigger: '.modal-body .o_existing_attachment_cell img[title="leewise_logo.png"]',
}, {
    ...
    // Check leewise logo is not shown as the search query does not match its name
    // and remove search query
    trigger: '.modal-body input.o_we_search',
    extra_trigger: '.modal-body .o_nocontent_help',
    run: 'remove_text',
}, {
    // Check that leewise logo is now shown in the cover selector, and make the trash
    // button visible
    trigger: '.modal-body .o_existing_attachment_cell img[title="leewise_logo.png"]',
    run() {},
}, {
    // Click on delete cover button
    trigger: '.modal-body .o_existing_attachment_cell:has(img[title="leewise_logo.png"]) .o_existing_attachment_remove',
    allowInvisible: true,
}, {
