...
patch(Wysiwyg.prototype, {
    ...
    appendBehaviorBlueprint(behaviorBlueprint) {
        const restoreSelection = () => {
            ...
            return setCursorEnd(this.leewiseEditor.editable, false);
        }
        const insert = (anchor) => {
            const fragment = this.leewiseEditor.document.createDocumentFragment();
            // Add a P after the Behavior to be able to continue typing
            // after it
            const p = this.leewiseEditor.document.createElement('p');
            p.append(this.leewiseEditor.document.createElement('br'));
            fragment.append(anchor, p);
            const insertedNodes = this.leewiseEditor.execCommand('insert', fragment);
            if (insertedNodes) {
    ...
    _insertItemCalendar: function () {
        const { ItemCalendarPropsDialog } = leewise.loader.modules.get("@knowledge/components/item_calendar_props_dialog/item_calendar_props_dialog");
        const restoreSelection = preserveCursor(this.leewiseEditor.document);
