...
const HtmlFieldPatch = {
    ...
    get valueContainerElement() {
        ...
        } else if (this.wysiwyg?.leewiseEditor) {
            return this.wysiwyg.leewiseEditor.editable;
        }
        return null;
    },
     ...
     * LeewiseEditor has no hook to tell if a node will be removed or not.
    ...
    async _mountBehaviors(behaviorsData, target) {
        ...
        for (const behaviorData of behaviorsData) {
            ...
            // Extract the configuration from the Leewise main Owl App to use it
    ...
    _prepareBehaviorAnchor(behaviorData, renderingContainerElement) {
        ...
        if (this.props.readonly) {
            ...
            behaviorData.insert = (anchor) => {
                if (this.wysiwyg.leewiseEditor.editable.contains(behaviorData.anchor)) {
                    // Ignore the insertion if the mounted element
                    // cannot be moved in the DOM.
                    this.wysiwyg.leewiseEditor.observerUnactive('mount_knowledge_behaviors');
                    ...
                    // manually.
                    this.wysiwyg.leewiseEditor.idSet(anchor);
                    this.wysiwyg.leewiseEditor.observerActive('mount_knowledge_behaviors');
                    return [anchor];
                }
            };
        }
        ...
        if (!this.props.readonly) {
            ...
            props.onReadyToInsertInEditor = () => {
                ...
                const insertedNodes = behaviorData.insert ?
                    behaviorData.insert(anchor) :
                    this.wysiwyg.leewiseEditor.execCommand('insert', anchor);
                if (!insertedNodes) {
                    return cancelInsertion();
                }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await leewise.loader.modules.get('@knowledge/js/wysiwyg');
        if (!wysiwygModule) {
    ...
    _addMountBehaviorsListeners() {
        if (this.wysiwyg?.leewiseEditor?.editable) {
            this.wysiwyg.leewiseEditor.editable.addEventListener('mount_knowledge_behaviors', this.boundMountBehaviors);
        }
    },
    ...
    _removeMountBehaviorsListeners() {
        if (this.wysiwyg?.leewiseEditor?.editable) {
            this.wysiwyg.leewiseEditor.editable.removeEventListener('mount_knowledge_behaviors', this.boundMountBehaviors);
        }
    },
};
