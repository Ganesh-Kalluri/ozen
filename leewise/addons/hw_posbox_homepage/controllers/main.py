...
from leewise import http, service, tools
from leewise.http import Response, request
from leewise.addons.hw_drivers.connection_manager import connection_manager
from leewise.addons.hw_drivers.main import iot_devices
from leewise.addons.hw_drivers.tools import helpers
from leewise.addons.web.controllers.home import Home
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('leewise.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(Home):
    ...
    def clean_partition(self):
        subprocess.check_call(['sudo', 'bash', '-c', '. /home/pi/leewise/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup'])

    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('leewise-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_leewise_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'leewise-remote-server.conf'
    ...
    def list_handlers(self, **post):
        ...
        if request.httprequest.method == 'POST':
            ...
            for post_request_key, log_level_or_parent in post.items():
                ...
                elif post_request_key == 'leewise':
                    need_config_save |= self._update_logger_level('leewise', log_level_or_parent, AVAILABLE_LOG_LEVELS)
        ...
        return handler_list_template.render({
            'title': "Leewise's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_leewise_server_url(),
            'root_logger_log_level': self._get_logger_effective_level_str(logging.getLogger()),
            'leewise_current_log_level': self._get_logger_effective_level_str(logging.getLogger('leewise')),
    ...
    def list_credential(self):
        return list_credential_template.render({
            'title': "Leewise's IoT Box - List credential",
            'breadcrumb': 'List credential',
            'db_uuid': helpers.read_file_first_line('leewise-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('leewise-enterprise-code.conf'),
        })

    @http.route('/save_credential', type='http', auth='none', cors='*', csrf=False)
    def save_credential(self, db_uuid, enterprise_code):
        helpers.write_file('leewise-db-uuid.conf', db_uuid)
        helpers.write_file('leewise-enterprise-code.conf', enterprise_code)
        helpers.leewise_restart(0)
    ...
    def clear_credential(self):
        helpers.unlink_file('leewise-db-uuid.conf')
        helpers.unlink_file('leewise-enterprise-code.conf')
        helpers.leewise_restart(0)
        return "<meta http-equiv='refresh' content='20; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        if server:
            res_payload['server'] = {
                'url': server,
                'message': 'Redirect to Leewise Server'
            }
        else:
            res_payload['server'] = {
    ...
    def clear_server_configuration(self):
        helpers.unlink_file('leewise-remote-server.conf')
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_leewise_server_url()
    ...
    def step_by_step_configure_page(self):
        return configure_wizard_template.render({
            ...
            'ssid': helpers.get_wifi_essid(),
            'server': helpers.get_leewise_server_url() or '',
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> Leewise server configuration',
            'breadcrumb': 'Configure Leewise Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_leewise_server_url() or 'Not configured yet',
    ...
    def add_six_payment_terminal(self, terminal_id):
        if terminal_id.isdigit():
            helpers.write_file('leewise-six-payment-terminal.conf', terminal_id)
    ...
    def clear_six_payment_terminal(self):
        helpers.unlink_file('leewise-six-payment-terminal.conf')
        service.server.restart()
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"

    @http.route('/hw_proxy/upgrade', type='http', auth='none', )
    def upgrade(self):
        commit = subprocess.check_output(["git", "--work-tree=/home/pi/leewise/", "--git-dir=/home/pi/leewise/.git", "log", "-1"]).decode('utf-8').replace("\n", "<br/>")
        ...
        return upgrade_page_template.render({
            'title': "Leewise's IoTBox - Software Upgrade",
    ...
    def perform_flashing_create_partition(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/leewise/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition']).decode().split('\n')[-2]
    ...
    def perform_flashing_download_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/leewise/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspios']).decode().split('\n')[-2]
    ...
    def perform_flashing_copy_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/leewise/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspios']).decode().split('\n')[-2]
    ...
    @http.route('/iot_restart_leewise_or_reboot', type='json', auth='none', cors='*', csrf=False)
    def iot_restart_leewise_or_reboot(self, action):
        """ Reboots the IoT Box / restarts Leewise on it depending on chosen 'action' argument"""
        try:
            if action == 'restart_leewise':
                helpers.leewise_restart(3)
            else:
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get Leewise Iot logger given an IoT handler name
        ...
        """
        leewise_addon_handler_path = helpers.compute_iot_handlers_addon_name(handler_folder_name, handler_name)
        return leewise_addon_handler_path in logging.Logger.manager.loggerDict.keys() and \
               logging.getLogger(leewise_addon_handler_path)

    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """
        Update (if necessary) Leewise's configuration and logger to the given logger_name to the given level.
        ...
        LEEWISE_TOOL_CONFIG_HANDLER_NAME = 'log_handler'
        LOG_HANDLERS = tools.config[LEEWISE_TOOL_CONFIG_HANDLER_NAME]
        ...
        tools.config[LEEWISE_TOOL_CONFIG_HANDLER_NAME] = log_handlers_without_logger
        new_level_upper_case = new_level.upper()
        if not IS_NEW_LEVEL_PARENT:
            new_entry = [LOGGER_PREFIX + new_level_upper_case]
            tools.config[LEEWISE_TOOL_CONFIG_HANDLER_NAME] += new_entry
            _logger.debug('Adding to leewise config log_handler: %s', new_entry)
