...
class StudioApprovalRule(models.Model):
    ...
    def set_approval(self, res_id, approved):
        ...
        :rtype: boolean
        :raise: leewise.exceptions.AccessError when the user does not have write
                access to the underlying record
        :raise: leewise.exceptions.UserError when any of the other checks failed
        """
        self.ensure_one()
    ...
    def delete_approval(self, res_id):
        ...
        :raise: leewise.exceptions.AccessError when the user does not have write
                access to the underlying record
        :raise: leewise.exceptions.UserError when any there is no existing entry
    ...
    def _set_approval(self, res_id, approved):
        ...
        :rtype: :class:`~leewise.addons.web_studio.models.StudioApprovalEntry`
        :raise: leewise.exceptions.AccessError when the user does not have write
                access to the underlying record
        :raise: leewise.exceptions.UserError when any of the other checks failed
        """
        ...
        # Leewise itself (not PG); the NOWAIT ensures that no deadlock will happen
