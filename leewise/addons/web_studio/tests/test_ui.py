...
from leewise import Command, api, http
...
class TestUi(leewise.tests.HttpCase):
    ...
    def test_new_app_and_report(self):
        if not leewise.tests.loaded_demo_data(self.env):
...
def _get_studio_view(view):
    domain = [('inherit_id', '=', view.id), ('name', '=', "Leewise Studio: %s customization" % (view.name))]
...
@leewise.tests.tagged('post_install', '-at_install')
class TestStudioUIUnit(leewise.tests.HttpCase):
    ...
    def test_edit_with_xml_editor(self):
        studioView = self.env["ir.ui.view"].create({
            ...
            'name': "Leewise Studio: %s customization" % (self.testView.name)
        })
    ...
    def test_context_write_cleaned(self):
        ...
        with mute_logger("leewise.addons.base.models.ir_ui_view"):
