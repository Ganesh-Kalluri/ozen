...
import { setSelection, startPos, endPos } from "@web_editor/js/editor/leewise-editor/src/utils/utils";
    ...
    return class CustomWysiwyg extends Wysiwyg {
        ...
        _showImageCrop() {
            ...
            const onDestroyed = async () => {
                ...
                this.focus();
                this.leewiseEditor.toolbarShow();
            };
...
export class ReportEditorWysiwyg extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            const Wysiwyg = (
                await leewise.loader.modules.get("@web_editor/js/wysiwyg/wysiwyg")
            ).Wysiwyg;
        ...
        this.setWysiwygInstance = async (wysiwyg) => {
            ...
            const leewiseEditor = this.wysiwyg.leewiseEditor;

            const undoRedoState = this.undoRedoState;
            undoRedoState.canUndo = false;
            undoRedoState.canRedo = false;

            leewiseEditor.addEventListener("historyStep", () => {
                undoRedoState.canUndo = leewiseEditor.historyCanUndo();
                undoRedoState.canRedo = leewiseEditor.historyCanRedo();
            ...
            this.observer = new MutationObserver((records) =>
                this.domChangesDirtyMutations(leewiseEditor, records)
            );
            leewiseEditor.addEventListener("observerUnactive", () => {
                if (this.observer) {
                    this.domChangesDirtyMutations(leewiseEditor, this.observer.takeRecords());
                    this.observer.disconnect();
                }
            });

            leewiseEditor.addEventListener("observerActive", observe);

            leewiseEditor.observerUnactive();
            if (leewise.debug) {
                ["t-esc", "t-out", "t-field"].forEach((tAtt) => {
                    leewiseEditor.document.querySelectorAll(`*[${tAtt}]`).forEach((e) => {
            ...
            leewiseEditor.observerActive();
        };

        this.undoRedoState = reactive({
            canUndo: false,
            canRedo: false,
            undo: () => this.wysiwyg?.leewiseEditor.historyUndo(),
            redo: () => this.wysiwyg?.leewiseEditor.historyRedo(),
        });
    }
    ...
    async discard() {
        this.wysiwyg.leewiseEditor.document.getSelection().removeAllRanges();
        await this.wysiwyg.cancel(false);
        await this.reportEditorModel.discardReport();
    }

    domChangesDirtyMutations(leewiseEditor, records) {
        if (this.wysiwyg.savingContent) {
            return;
        }
        records = leewiseEditor.filterMutationRecords(records);
    ...
    getFieldPopoverParams() {
        const leewiseEditor = this.wysiwyg.leewiseEditor;
        const doc = leewiseEditor.document;
    ...
    async insertTableX2Many() {
        ...
        await this.fieldPopover.open(popoverAnchor, {
            ...
            validate: (
                ...
                this.wysiwyg.focus();
                const doc = this.wysiwyg.leewiseEditor.document;
                ...
                this.wysiwyg.leewiseEditor.execCommand("insert", table);
    ...
    async insertField() {
        ...
        await this.fieldPopover.open(popoverAnchor, {
            ...
            validate: (qwebVar, fieldNameChain, defaultValue = "", is_image) => {
                ...
                this.wysiwyg.leewiseEditor.execCommand("insert", span);
            },
        });
    }
