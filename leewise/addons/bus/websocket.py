...
from leewise.http import root, Request, Response, SessionExpiredException, get_default_session
from leewise.modules.registry import Registry
from leewise.service import model as service_model
from leewise.service.server import CommonServer
from leewise.service.security import check_session
...
def acquire_cursor(db):
    ...
    for tryno in range(1, MAX_TRY_ON_POOL_ERROR + 1):
        with suppress(PoolError):
            return leewise.registry(db).cursor()
...
class Websocket:
    ...
    def __init__(self, sock, session):
        ...
        self.__selector = (
            ...
            if leewise.evented and hasattr(selectors, 'PollSelector')
...
class WebsocketConnectionHandler:
    ...
    def _handle_public_configuration(cls, request):
        if not os.getenv('LEEWISE_BUS_PUBLIC_SAMESITE_WS'):
            return
