...
from leewise.tools.misc import DEFAULT_SERVER_DATETIME_FORMAT
from leewise.tools import date_utils
        ...
        if channels:
            ...
            def notify():
                with leewise.sql_db.db_connect('postgres').cursor() as cr:
                    query = sql.SQL("SELECT {}('imbus', %s)").format(sql.Identifier(LEEWISE_NOTIFY_FUNCTION))
