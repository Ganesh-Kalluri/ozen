...
export const presenceService = {
    start(env) {
        ...
        function onFocusChange(isFocused) {
            ...
            isLeewiseFocused = isFocused;
            browser.localStorage.setItem(`${LOCAL_STORAGE_PREFIX}.focus`, isLeewiseFocused);
            if (isLeewiseFocused) {
                lastPresenceTime = new Date().getTime();
                env.bus.trigger("window_focus", isLeewiseFocused);
            }
        }

        function onStorage({ key, newValue }) {
            if (key === `${LOCAL_STORAGE_PREFIX}.focus`) {
                isLeewiseFocused = JSON.parse(newValue);
