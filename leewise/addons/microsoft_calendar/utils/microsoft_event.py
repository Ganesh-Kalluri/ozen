...
from leewise.api import model
from typing import Iterator, Mapping
from collections import abc
from leewise.tools import ReadonlyDict, email_normalize
...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by Leewise recordset, one instance can be a single Microsoft event or a
    ...
    def leewise_id(self, env):
        return self._leewise_id

    def _meta_leewise_id(self, microsoft_guid):
        """Returns the Leewise id stored in the Microsoft Event metadata.
        This id might not actually exists in the database.
        """
        return None

    @property
    def leewise_ids(self):
        """
        Get the list of Leewise event ids already mapped with Outlook events (self)
        """
        return tuple(e._leewise_id for e in self if e._leewise_id)

    def _load_leewise_ids_from_db(self, env, force_model=None):
        """
        Map Microsoft events to existing Leewise events:
        1) extract unmapped events only,
        2) match Leewise events and Outlook events which have both a ICalUId set,
        3) match remaining events,
        Returns the list of mapped events
        """
        mapped_events = [e.id for e in self if e._leewise_id]
        ...
        res = env.cr.fetchall()
        leewise_events_ids = [val[0] for val in res]
        leewise_events = model_env.browse(leewise_events_ids)

        # 1. try to match unmapped events with Leewise events using their iCalUId
        unmapped_events_with_uids = unmapped_events.filter(lambda e: e.iCalUId)
        leewise_events_with_uids = leewise_events.filtered(lambda e: e.ms_universal_event_id)
        mapping = {e.ms_universal_event_id: e.id for e in leewise_events_with_uids}

        for ms_event in unmapped_events_with_uids:
            leewise_id = mapping.get(ms_event.iCalUId)
            if leewise_id:
                ms_event._events[ms_event.id]['_leewise_id'] = leewise_id
                mapped_events.append(ms_event.id)

        # 2. try to match unmapped events with Leewise events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        mapping = {e.ms_organizer_event_id: e for e in leewise_events}

        for ms_event in unmapped_events:
            leewise_event = mapping.get(ms_event.id)
            if leewise_event:
                ms_event._events[ms_event.id]['_leewise_id'] = leewise_event.id
                mapped_events.append(ms_event.id)

                # don't forget to also set the global event ID on the Leewise event to ease
    ...
    def owner_id(self, env):
        ...
        1) the current Leewise user is the organizer of the event according to Outlook event, so return his id.
        2) the current Leewise user is NOT the organizer and:
           2.1) we are able to find a Leewise user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a Leewise user matching the organizer email address and we return False, meaning
                that no Leewise user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        ...
        if organizer_email:
            # Warning: In Microsoft: 1 email = 1 user; but in Leewise several users might have the same email
    ...
    def match_with_leewise_events(self, env) -> 'MicrosoftEvent':
        """
        Match Outlook events (self) with existing Leewise events, and return the list of matched events
        """
        ...
        mapped_recurrences = recurrence_candidates._load_leewise_ids_from_db(env, force_model=env["calendar.recurrence"])

        # then, try to match events
        events_candidates = (self - mapped_recurrences).filter(lambda x: not x.is_recurrence())
        mapped_events = events_candidates._load_leewise_ids_from_db(env)
