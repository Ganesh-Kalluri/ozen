...
import logging
import requests
from leewise.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
from datetime import timedelta

from leewise import api, fields, models, _, Command
from leewise.exceptions import UserError
from leewise.loglevels import exception_to_unicode
from leewise.addons.microsoft_account.models.microsoft_service import DEFAULT_MICROSOFT_TOKEN_ENDPOINT
from leewise.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Leewise
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2leewise(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Leewise -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_leewise2microsoft()
        ...
        (events - synced_events)._sync_leewise2microsoft()
        self.microsoft_last_sync_date = fields.datetime.now()
