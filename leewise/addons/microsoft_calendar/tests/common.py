...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Leewise event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Leewise events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Leewise events
        # -----------------------------------------------------------------------------------------

        self.expected_leewise_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_leewise_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_leewise_recurrency_events_from_outlook = [
            {
    ...
    def sync_leewise_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Leewise. A trick for deactivating the checking
    ...
    def assert_leewise_event(self, leewise_event, expected_values):
        """
        Assert that an Leewise event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        leewise_event_values = leewise_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(leewise_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, leewise_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_leewise_recurrence(self, leewise_recurrence, expected_values):
        """
        Assert that an Leewise recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        leewise_recurrence_values = leewise_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, leewise_recurrence_values.get(k), msg=f"'{k}' mismatch")
