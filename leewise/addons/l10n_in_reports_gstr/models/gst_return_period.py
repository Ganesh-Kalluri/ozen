...
from leewise.addons.iap import jsonrpc
from leewise.exceptions import UserError, AccessError, ValidationError, RedirectWarning
from leewise.tools import date_utils, get_lang, html_escape
from leewise.tools.misc import format_date
from leewise.addons.l10n_in_edi.models.account_edi_format import DEFAULT_IAP_ENDPOINT, DEFAULT_IAP_TEST_ENDPOINT
    ...
    def check_gstr1_status(self):
        ...
        if response.get('data'):
            ...
            if data.get("status_cd") == "P":
                ...
                self.sudo().message_post(body=_("GSTR-1 Successfully Sent"), author_id=leewisebot.id)
            ...
            for gstr2b_bill in gstr2b_streamline_bills:
                ...
                if matched_bills:
                    created_from_reconciliation = matched_bills.filtered(lambda b:
                        b.l10n_in_gstr2b_reconciliation_status == 'gstr2_bills_not_in_leewise' and b.state == 'draft')
                    ...
                    create_vals.append({
                        ...
                        "l10n_in_gstr2b_reconciliation_status": "gstr2_bills_not_in_leewise",
                        "to_check": True,
                        "l10n_in_gst_return_period_id": self.id,
                        "message_ids":[(0, 0, {
                            'model': 'account.move',
