...
export const preparationDisplayService = {
    ...
    async start(env, { orm, bus_service, sound }) {
        const datas = await orm.call(
            ...
            [[leewise.preparation_display.id]],
            {}
        );
        ...
        bus_service.addChannel(`preparation_display-${leewise.preparation_display.access_token}`);
        ...
        bus_service.addEventListener("notification", async (message) => {
            const proms = message.detail.map((detail) => {
                ...
                if (datas.preparation_display_id !== leewise.preparation_display.id) {
                    return false;
                }
                switch (detail.type) {
                    case "load_orders":
