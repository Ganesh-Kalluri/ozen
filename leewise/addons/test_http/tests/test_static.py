...
import leewise
from leewise.tests import new_test_user, tagged, RecordCapturer
from leewise.tools import config, file_open, image_process
...
class TestHttpStaticLogo(TestHttpStaticCommon):
    ...
    def setUpClass(cls):
        ...
        super_user = cls.env['res.users'].browse([leewise.SUPERUSER_ID])
    ...
    def _test_upload_small_file(self):
        ...
        with RecordCapturer(self.env['ir.attachment'], []) as capture, \
            ...
            res = self.opener.post(
                ...
                files={'ufile': file},
                data={
                    'csrf_token': leewise.http.Request.csrf_token(self),
