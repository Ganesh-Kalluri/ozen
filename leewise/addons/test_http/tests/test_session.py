...
import leewise
from leewise.tests.common import get_db_name
from leewise.tools import mute_logger
from .test_common import TestHttpBase


GEOIP_LEEWISE_FARM_2 = {
    'city': 'Ramillies',
    'country_code': 'BE',
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('leewise.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session1_default_session(self):
        ...
        with patch.object(leewise.http.root.session_store, 'save') as mock_save:
    ...
    def test_session3_logout_15_0_geoip(self):
        ...
        with self.assertLogs('leewise.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:leewise.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    def test_session7_serializable(self):
        ...
        # Values forbidden by leewise, raising a warning
        for value in [
            str,
            int,
            float,
            ...
            with self.assertLogs(level="WARNING"):
                ...
                self.assertFalse(leewise.http.root.session_store.session_class({'foo': value}, 1234).foo)
