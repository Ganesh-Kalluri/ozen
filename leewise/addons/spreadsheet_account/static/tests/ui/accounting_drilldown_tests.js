...
QUnit.module("spreadsheet_account > Accounting Drill down", { beforeEach }, () => {
    QUnit.test("Create drill down domain", async (assert) => {
        ...
        env.model = model;
        setCellContent(model, "A1", `=LEEWISE.BALANCE("100", 2020)`);
        setCellContent(model, "A2", `=LEEWISE.BALANCE("100", 0)`);
        setCellContent(model, "A3", `=LEEWISE.BALANCE("100", 2020, , , FALSE)`);
        setCellContent(model, "A4", `=LEEWISE.BALANCE("100", 2020, , , )`);
