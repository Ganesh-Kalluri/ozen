...
from leewise.addons.website_crm_partner_assign.controllers.main import WebsiteCrmPartnerAssign
...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('leewise.addons.base_geolocalize.models.base_geocoder.GeoCoder.geo_find', wraps=geo_find)
...
class TestPartnerLeadPortal(TestCrmCommon):
    ...
    @patch('leewise.http.GeoIP')
    def test_03_crm_partner_assign_geolocalization(self, GeoIpMock):
        """
            This test checks situation when "{LeewiseURL}/partners" is visited from foreign country without resellers.
        ...
        """
        # Patch GeoIp so it acts, as if Leewise client is located in Mexico
