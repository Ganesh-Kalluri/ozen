...
patch(PosStore.prototype, {
     ...
     * send it to Leewise. Two cases can happen:
     * - Failure to send to Fiskaly => we assume that if one order fails, EVERY order will fail
     * - Failure to send to Leewise => the order is already sent to Fiskaly, we store them locally with the TSS info
     */
    async _flush_orders(orders, options) {
        ...
        if (result && fiskalyFailure.length === 0) {
            ...
            throw leewiseError || fiskalyError;
        }
    },
