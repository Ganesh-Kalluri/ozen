...
export const LEEWISE_VERSION_KEY = `${location.origin.replace(
    /:\/{0,2}/g,
    "_"
)}_im_livechat.leewise_version`;
...
export class LivechatService {
    ...
    async initialize() {
        ...
        if (!this.options.isTestChatbot) {
            ...
            const prevLeewiseVersion = browser.localStorage.getItem(LEEWISE_VERSION_KEY);
            const currLeewiseVersion = init?.leewise_version;
            const visitorUid = this.visitorUid || false;
            const userId = session.user_id || false;
            if (
                prevLeewiseVersion !== currLeewiseVersion ||
                (this.sessionCookie && visitorUid !== userId)
            ) {
                this.leaveSession({ notifyServer: false });
            }
            browser.localStorage.setItem(LEEWISE_VERSION_KEY, currLeewiseVersion);
        }
