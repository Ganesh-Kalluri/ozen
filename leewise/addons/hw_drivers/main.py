...
from leewise.addons.hw_drivers.websocket_client import WebsocketClient
...
class Manager(Thread):
    def send_alldevices(self, iot_client=None):
        """
        This method send IoT Box and devices informations to Leewise database
        """
        server = helpers.get_leewise_server_url()
        if server:
            subject = helpers.read_file_first_line('leewise-subject.conf')
            if subject:
            ...
            _logger.warning('Leewise server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the leewise server with the updates
        """

        helpers.start_nginx_server()
        _logger.info("IoT Box Image version: %s", helpers.get_version())
        if platform.system() == 'Linux' and helpers.get_leewise_server_url():
        ...
        iot_client = helpers.get_leewise_server_url() and WebsocketClient(helpers.get_leewise_server_url())
        ...
        if helpers.get_leewise_server_url():
            iot_client.start()
