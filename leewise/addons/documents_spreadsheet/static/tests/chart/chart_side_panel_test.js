...
QUnit.module("documents_spreadsheet > chart side panel", { beforeEach }, () => {
    ...
    QUnit.test("From an Leewise chart, can only change to an Leewise chart", async (assert) => {
        const { model, env } = await createSpreadsheetFromGraphView();
        await openChartSidePanel(model, env);
        const target = getFixture();
        ...
        assert.strictEqual(options[0].value, "leewise_bar");
        assert.strictEqual(options[1].value, "leewise_line");
        assert.strictEqual(options[2].value, "leewise_pie");
    });
    ...
    QUnit.test("Change leewise chart type", async (assert) => {
        const { model, env } = await createSpreadsheetFromGraphView();
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
        assert.strictEqual(model.getters.getChart(chartId).type, "leewise_bar");
        await openChartSidePanel(model, env);
        const target = getFixture();
        /** @type {HTMLSelectElement} */
        const select = target.querySelector(".o-type-selector");
        select.value = "leewise_pie";
        await triggerEvent(select, null, "change");
        assert.strictEqual(model.getters.getChart(chartId).type, "leewise_pie");
        select.value = "leewise_line";
        await triggerEvent(select, null, "change");
        assert.strictEqual(model.getters.getChart(chartId).type, "leewise_line");
    ...
    QUnit.test("Open chart leewise's data properties", async function (assert) {
        const target = getFixture();
        const { model, env } = await createSpreadsheetFromGraphView();
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
