...
QUnit.module(
    ...
    () => {
        QUnit.test(
            "can link an leewise menu to a basic chart chart in the side panel",
            async function (assert) {
                ...
                let leewiseMenu = model.getters.getChartLeewiseMenu(chartId);
                assert.equal(leewiseMenu, undefined, "No menu linked with chart at start");

                const irMenuField = target.querySelector(".o-ir-menu-selector input");
                assert.ok(
                    irMenuField,
                    "A menu to link charts to leewise menus was added to the side panel"
                );
                await click(irMenuField);
                await editInput(irMenuField, null, "");
                await click(document.querySelectorAll(".ui-menu-item")[0]);
                leewiseMenu = model.getters.getChartLeewiseMenu(chartId);
                assert.equal(
                    leewiseMenu.xmlid,
                    "documents_spreadsheet.test.menu",
                    "Leewise menu is linked to chart"
                );
            }
        );

        QUnit.test(
            "can link an leewise menu to a scorecard chart chart in the side panel",
            async function (assert) {
                const { model } = await createSpreadsheet({
                    serverData: this.serverData,
                });
        ...
        QUnit.test(
            "can link an leewise menu to a gauge chart chart in the side panel",
            async function (assert) {
                const { model } = await createSpreadsheet({
                    serverData: this.serverData,
                });
        ...
        QUnit.test(
            "can remove link between an leewise menu and a chart in the side panel",
            async function (assert) {
                const { model } = await createSpreadsheet({
                    serverData: this.serverData,
                });
                ...
                model.dispatch("LINK_LEEWISE_MENU_TO_CHART", {
                    chartId,
                    leewiseMenuId: "documents_spreadsheet.test.menu",
                });
                await openChartSidePanel();
                await nextTick();
                const irMenuField = target.querySelector(".o-ir-menu-selector input");
                ...
                assert.equal(leewiseMenu, undefined, "no menu is linked to chart");
            }
        );
