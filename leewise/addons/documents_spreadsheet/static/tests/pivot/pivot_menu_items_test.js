    ...
    function () {
        QUnit.test("Reinsert a pivot", async function (assert) {
            ...
            assert.equal(
                ...
                `=LEEWISE.PIVOT(1,"probability","bar","false","foo",1)`,
        ...
        QUnit.test("Reinsert a pivot with a contextual search domain", async function (assert) {
            ...
            assert.equal(
                getCellFormula(model, "E10"),
                `=LEEWISE.PIVOT(1,"probability","bar","false","foo",${uid})`,
                "It should contain a pivot formula"
            );
        });
        ...
        QUnit.test("Reinsert a pivot with new data", async function (assert) {
            ...
            selectCell(model, "D8");
            await doMenuAction(topbarMenuRegistry, reinsertPivotPath, env);
            assert.equal(getCellFormula(model, "I8"), `=LEEWISE.PIVOT.HEADER(1,"foo",25)`);
            assert.equal(
                getCellFormula(model, "I10"),
                `=LEEWISE.PIVOT(1,"probability","bar","false","foo",25)`
            );
        });
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                await doMenuAction(topbarMenuRegistry, reinsertPivotPath, env);
                assert.equal(getCellFormula(model, "C1"), `=LEEWISE.PIVOT.HEADER(1,"foo",2)`);
                assert.equal(
                    getCellFormula(model, "C2"),
                    `=LEEWISE.PIVOT.HEADER(1,"foo",2,"measure","probability")`
                );
                assert.equal(
        ...
        QUnit.test("Verify absence of pivot properties on formula with invalid pivot Id", async function (assert) {
            const { model, env } = await createSpreadsheetWithPivot();
            setCellContent(model, "A1", `=LEEWISE.PIVOT.HEADER("fakeId")`);
            const root = cellMenuRegistry.getAll().find((item) => item.id === "pivot_properties");
            assert.notOk(root.isVisible(env));
            setCellContent(model, "A1", `=LEEWISE.PIVOT("fakeId", "probability", "foo", 2)`);
            assert.notOk(root.isVisible(env));
        });
        ...
        QUnit.test(
            "Can rebuild the Leewise domain of records based on the according merged pivot cell",
            async function (assert) {
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                setCellContent(
                    model,
                    "B4",
                    getCellFormula(model, "B4").replace("LEEWISE.PIVOT", "leewise.pivot")
                );
                assert.ok(root.isVisible(env));
            }
        );
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                setCellContent(
                    model,
                    "B4",
                    getCellFormula(model, "B4").replace(`LEEWISE.PIVOT(1`, `LEEWISE.PIVOT("5)`)
                ); //Invalid id
