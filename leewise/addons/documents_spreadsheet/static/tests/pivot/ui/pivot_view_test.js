...
QUnit.module("spreadsheet pivot view", {}, () => {
    QUnit.test("simple pivot export", async (assert) => {
        ...
        assert.strictEqual(getCellContent(model, "A3"), "=LEEWISE.PIVOT.HEADER(1)");
        assert.strictEqual(getCellContent(model, "B1"), "=LEEWISE.PIVOT.HEADER(1)");
        assert.strictEqual(getCellContent(model, "B2"), '=LEEWISE.PIVOT.HEADER(1,"measure","foo")');
        assert.strictEqual(getCellContent(model, "B3"), '=LEEWISE.PIVOT(1,"foo")');
    });
    ...
    QUnit.test("simple pivot export with two measures", async (assert) => {
        ...
        assert.strictEqual(
            ...
            '=LEEWISE.PIVOT.HEADER(1,"measure","probability")'
        );
        assert.strictEqual(getCellContent(model, "B3"), '=LEEWISE.PIVOT(1,"foo")');
        assert.strictEqual(getCellContent(model, "C3"), '=LEEWISE.PIVOT(1,"probability")');
    });
    ...
    QUnit.test("groupby date field without interval defaults to month", async (assert) => {
        ...
        assert.equal(getCellContent(model, "A3"), '=LEEWISE.PIVOT.HEADER(1,"date","04/2016")');
        assert.equal(getCellContent(model, "A4"), '=LEEWISE.PIVOT.HEADER(1,"date","10/2016")');
        assert.equal(getCellContent(model, "A5"), '=LEEWISE.PIVOT.HEADER(1,"date","12/2016")');
        assert.equal(
            getCellContent(model, "B3"),
            '=LEEWISE.PIVOT(1,"probability","date","04/2016","foo",1)'
        );
        assert.equal(
            getCellContent(model, "B4"),
            '=LEEWISE.PIVOT(1,"probability","date","10/2016","foo",1)'
        );
        assert.equal(
            getCellContent(model, "B5"),
            '=LEEWISE.PIVOT(1,"probability","date","12/2016","foo",1)'
        );
    ...
    QUnit.test("pivot with one level of group bys", async (assert) => {
        ...
        assert.strictEqual(getCellContent(model, "A3"), '=LEEWISE.PIVOT.HEADER(1,"bar","false")');
        assert.strictEqual(getCellContent(model, "A4"), '=LEEWISE.PIVOT.HEADER(1,"bar","true")');
        assert.strictEqual(getCellContent(model, "A5"), "=LEEWISE.PIVOT.HEADER(1)");
        assert.strictEqual(
            getCellContent(model, "B2"),
            '=LEEWISE.PIVOT.HEADER(1,"foo",1,"measure","probability")'
        );
        assert.strictEqual(
            getCellContent(model, "C3"),
            '=LEEWISE.PIVOT(1,"probability","bar","false","foo",2)'
        );
        assert.strictEqual(getCellContent(model, "F5"), '=LEEWISE.PIVOT(1,"probability")');
    });
    ...
    QUnit.test("pivot with two levels of group bys in rows", async (assert) => {
        ...
        assert.strictEqual(
            ...
            '=LEEWISE.PIVOT.HEADER(1,"bar","false","product_id",41)'
        );
        assert.deepEqual(getCell(model, "A4").style, { fillColor: "#E6F2F3" });
        assert.strictEqual(getCellContent(model, "A5"), '=LEEWISE.PIVOT.HEADER(1,"bar","true")');
        assert.strictEqual(
            getCellContent(model, "A6"),
            '=LEEWISE.PIVOT.HEADER(1,"bar","true","product_id",37)'
        );
        assert.strictEqual(
            getCellContent(model, "A7"),
            '=LEEWISE.PIVOT.HEADER(1,"bar","true","product_id",41)'
        );
        assert.strictEqual(getCellContent(model, "A8"), "=LEEWISE.PIVOT.HEADER(1)");
    });
    ...
    QUnit.test("verify that there is a record for an undefined many2one header", async (assert) => {
        ...
        assert.strictEqual(
            ...
            '=LEEWISE.PIVOT.HEADER(1,"product_id","false")'
        );
    });
    ...
    QUnit.test("undefined date is inserted in pivot", async (assert) => {
        ...
        assert.strictEqual(getCellContent(model, "A3"), '=LEEWISE.PIVOT.HEADER(1,"date:day","false")');
    });
    ...
    QUnit.test("pivot with two levels of group bys in cols", async (assert) => {
        ...
        assert.strictEqual(getCellContent(model, "B1"), '=LEEWISE.PIVOT.HEADER(1,"bar","false")');
        assert.strictEqual(
            getCellContent(model, "B2"),
            '=LEEWISE.PIVOT.HEADER(1,"bar","false","product_id",41)'
        );
        assert.strictEqual(
            getCellContent(model, "B3"),
            '=LEEWISE.PIVOT.HEADER(1,"bar","false","product_id",41,"measure","probability")'
        );
        assert.deepEqual(getCell(model, "C2").style, { fillColor: "#E6F2F3", bold: true });
        assert.strictEqual(getCellContent(model, "C1"), '=LEEWISE.PIVOT.HEADER(1,"bar","true")');
        assert.strictEqual(
            getCellContent(model, "C2"),
            '=LEEWISE.PIVOT.HEADER(1,"bar","true","product_id",37)'
        );
        assert.strictEqual(
            getCellContent(model, "C3"),
            '=LEEWISE.PIVOT.HEADER(1,"bar","true","product_id",37,"measure","probability")'
        );
        assert.strictEqual(
        ...
        assert.strictEqual(
            ...
            '=LEEWISE.PIVOT.HEADER(1,"bar","true","product_id",41,"measure","probability")'
        );
    });
    ...
    QUnit.test("pivot with count as measure", async (assert) => {
        ...
        assert.strictEqual(
            getCellContent(model, "C2"),
            '=LEEWISE.PIVOT.HEADER(1,"measure","__count")'
        );
        assert.strictEqual(getCellContent(model, "C3"), '=LEEWISE.PIVOT(1,"__count")');
    });
    ...
    QUnit.test("groupby week is sorted", async (assert) => {
        ...
        assert.strictEqual(
            getCellContent(model, "A3"),
            `=LEEWISE.PIVOT.HEADER(1,"date:week","15/2016")`
        );
        assert.strictEqual(
            getCellContent(model, "A4"),
            `=LEEWISE.PIVOT.HEADER(1,"date:week","43/2016")`
        );
        assert.strictEqual(
            getCellContent(model, "A5"),
            `=LEEWISE.PIVOT.HEADER(1,"date:week","49/2016")`
        );
        assert.strictEqual(
            getCellContent(model, "A6"),
            `=LEEWISE.PIVOT.HEADER(1,"date:week","50/2016")`
        );
    });
    ...
    QUnit.test("pivot with a domain", async (assert) => {
        ...
        assert.strictEqual(getCellContent(model, "A3"), `=LEEWISE.PIVOT.HEADER(1,"bar","true")`);
        assert.strictEqual(getCellContent(model, "A4"), `=LEEWISE.PIVOT.HEADER(1)`);
    });
    ...
    QUnit.test("pivot with a quote in name", async function (assert) {
        ...
        assert.equal(
            ...
            `=LEEWISE.PIVOT.HEADER(1,"display_name","name with \\"")`
        );
    });
    ...
    QUnit.test("group by related field with archived record", async function (assert) {
        ...
        assert.equal(getCellContent(model, "B1"), `=LEEWISE.PIVOT.HEADER(1,"product_id",37)`);
        assert.equal(getCellContent(model, "C1"), `=LEEWISE.PIVOT.HEADER(1,"product_id",41)`);
        assert.equal(getCellContent(model, "D1"), `=LEEWISE.PIVOT.HEADER(1)`);
    });
    ...
    QUnit.test("group by regular field with archived record", async function (assert) {
        ...
        assert.equal(getCellContent(model, "A3"), `=LEEWISE.PIVOT.HEADER(1,"foo",1)`);
        assert.equal(getCellContent(model, "A4"), `=LEEWISE.PIVOT.HEADER(1,"foo",2)`);
        assert.equal(getCellContent(model, "A5"), `=LEEWISE.PIVOT.HEADER(1,"foo",17)`);
        assert.equal(getCellContent(model, "A6"), `=LEEWISE.PIVOT.HEADER(1)`);
    });
    ...
    QUnit.test("search view with group by and additional row group", async (assert) => {
        ...
        assert.strictEqual(getCellContent(model, "A3"), '=LEEWISE.PIVOT.HEADER(1,"name","false")');
        assert.strictEqual(
            getCellContent(model, "A4"),
            '=LEEWISE.PIVOT.HEADER(1,"name","false","foo",1)'
        );
        assert.strictEqual(
            getCellContent(model, "A5"),
            '=LEEWISE.PIVOT.HEADER(1,"name","false","foo",2)'
        );
        assert.strictEqual(
            getCellContent(model, "A6"),
            '=LEEWISE.PIVOT.HEADER(1,"name","false","foo",12)'
        );
        assert.strictEqual(
            getCellContent(model, "A7"),
            '=LEEWISE.PIVOT.HEADER(1,"name","false","foo",17)'
        );
        assert.strictEqual(
            getCellContent(model, "B2"),
            '=LEEWISE.PIVOT.HEADER(1,"measure","__count")'
