...
import { registries, components } from "@leewise/o-spreadsheet";
...
async function openMenuSelector(params = {}) {
    ...
    await click(target, ".o-menu-item[data-name='leewiseMenu']");
...
QUnit.module("spreadsheet > menu link ui", { beforeEach }, () => {
    QUnit.test("insert a new ir menu link", async function (assert) {
        ...
        assert.equal(
            cell.content,
            "[menu with xmlid](leewise://ir_menu_xml_id/test_menu)",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.equal(
                cell.content,
                "[menu without xmlid](leewise://ir_menu_id/12)",
                "The content should be the complete markdown link"
            );
            assert.equal(
                target.querySelector(".o-link-tool a").text,
    ...
    QUnit.test("ir.menu link keep breadcrumb", async function (assert) {
        ...
        setCellContent(model, "A1", "[menu with xmlid](leewise://ir_menu_xml_id/test_menu)");
