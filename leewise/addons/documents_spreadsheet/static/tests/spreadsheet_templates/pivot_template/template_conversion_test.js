...
QUnit.module("documents_spreadsheet > pivot_templates", {}, function () {
    ...
    QUnit.test("Don't change formula if not many2one", async function (assert) {
        const formula = `LEEWISE.PIVOT("1","probability","foo","12","bar","110")`;
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            let result = await convertFormula({
                arch,
                formula: `LEEWISE.PIVOT("1","probability","product_id","37","bar","110")`,
                convert: "CONVERT_PIVOT_TO_TEMPLATE",
            });
            assert.equal(
                result,
                `LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id",1),"bar","110")`
            );

            result = await convertFormula({
                arch,
                formula: `LEEWISE.PIVOT.HEADER("1","product_id","37","bar","110")`,
                convert: "CONVERT_PIVOT_TO_TEMPLATE",
            });
            assert.equal(
                result,
                `LEEWISE.PIVOT.HEADER("1","product_id",LEEWISE.PIVOT.POSITION("1","product_id",1),"bar","110")`
            );

            result = await convertFormula({
                arch,
                formula: `LEEWISE.PIVOT("1","probability","product_id","41","bar","110")`,
                convert: "CONVERT_PIVOT_TO_TEMPLATE",
            });
            assert.equal(
                result,
                `LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id",2),"bar","110")`
            );

            result = await convertFormula({
                arch,
                formula: `LEEWISE.PIVOT.HEADER("1","product_id","41","bar","110")`,
                convert: "CONVERT_PIVOT_TO_TEMPLATE",
            });
            assert.equal(
                result,
                `LEEWISE.PIVOT.HEADER("1","product_id",LEEWISE.PIVOT.POSITION("1","product_id",2),"bar","110")`
            );
        }
    );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            let result = await convertFormula({
                arch,
                formula: `LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id", 1),"bar","110")`,
                convert: "CONVERT_PIVOT_FROM_TEMPLATE",
            });
            assert.equal(result, `LEEWISE.PIVOT("1","probability","product_id","37","bar","110")`);

            result = await convertFormula({
                arch,
                formula: `LEEWISE.PIVOT.HEADER("1","product_id",LEEWISE.PIVOT.POSITION("1","product_id",1),"bar","110")`,
                convert: "CONVERT_PIVOT_FROM_TEMPLATE",
            });
            assert.equal(result, `LEEWISE.PIVOT.HEADER("1","product_id","37","bar","110")`);

            result = await convertFormula({
                arch,
                formula: `LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id", 2),"bar","110")`,
                convert: "CONVERT_PIVOT_FROM_TEMPLATE",
            });
            assert.equal(result, `LEEWISE.PIVOT("1","probability","product_id","41","bar","110")`);

            result = await convertFormula({
                arch,
                formula: `LEEWISE.PIVOT.HEADER("1","product_id",LEEWISE.PIVOT.POSITION("1","product_id", 2),"bar","110")`,
                convert: "CONVERT_PIVOT_FROM_TEMPLATE",
            });
            assert.equal(result, `LEEWISE.PIVOT.HEADER("1","product_id","41","bar","110")`);
        }
    );
    ...
    QUnit.test("Will ignore overflowing template position", async function (assert) {
        ...
        const result = await convertFormula({
            arch,
            formula: `LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id", 9999),"bar","110")`,
            convert: "CONVERT_PIVOT_FROM_TEMPLATE",
        });
        assert.equal(result, "");
    });
    ...
    QUnit.test("Adapt pivot as function arg from relative to absolute", async function (assert) {
        ...
        assert.equal(
            result,
            `SUM(LEEWISE.PIVOT("1","probability","product_id","37","bar","110"),LEEWISE.PIVOT("1","probability","product_id","41","bar","110"))`
        );
    });
    ...
    QUnit.test("Adapt pivot as operator arg from relative to absolute", async function (assert) {
        ...
        assert.equal(
            result,
            `LEEWISE.PIVOT("1","probability","product_id","37","bar","110")+LEEWISE.PIVOT("1","probability","product_id","41","bar","110")`
        );
    });
    ...
    QUnit.test("Adapt pivot as operator arg from absolute to relative", async function (assert) {
        ...
        const result = await convertFormula({
            arch,
            formula: `
                LEEWISE.PIVOT("1","probability","product_id","37","bar","110")
                +
                LEEWISE.PIVOT("1","probability","product_id","41","bar","110")
            `,
            convert: "CONVERT_PIVOT_TO_TEMPLATE",
        });
        assert.equal(
            result,
            `LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id",1),"bar","110")+LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id",2),"bar","110")`
        );
    });
    ...
    QUnit.test("Adapt pivot as function arg from absolute to relative", async function (assert) {
        ...
        assert.equal(
            result,
            `SUM(LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id",1),"bar","110"),LEEWISE.PIVOT("1","probability","product_id",LEEWISE.PIVOT.POSITION("1","product_id",2),"bar","110"))`
        );
    });
    ...
    QUnit.test("Computed ids are not changed", async function (assert) {
        ...
        const result = await convertFormula({
            arch,
            formula: `LEEWISE.PIVOT("1","probability","product_id",A2,"bar","110")`,
            convert: "CONVERT_PIVOT_TO_TEMPLATE",
        });
        assert.equal(result, `LEEWISE.PIVOT("1","probability","product_id",A2,"bar","110")`);
    });
    ...
    QUnit.test(
        ...
        async (assert) => {
            ...
            assert.equal(cells.A4.content, "=LEEWISE.PIVOT.HEADER(1)");
            assert.equal(cells.B1.content, '=LEEWISE.PIVOT.HEADER(1,"foo",1)');
            assert.equal(cells.B2.content, '=LEEWISE.PIVOT.HEADER(1,"foo",1,"bar","true")');
            assert.equal(
                cells.B3.content,
                '=LEEWISE.PIVOT.HEADER(1,"foo",1,"bar","true","measure","probability")'
            );
            assert.equal(cells.B4.content, '=LEEWISE.PIVOT(1,"probability","foo",1,"bar","true")');
        }
    );
});
