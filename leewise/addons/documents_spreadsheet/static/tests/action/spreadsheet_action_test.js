...
QUnit.module(
    ...
    function () {
                ...
                sheets: [
                    {
                        ...
                        cells: {
                            A1: {
                                content:
                                    '=LEEWISE.PIVOT(1,"probability","foo", LEEWISE.PIVOT.POSITION(1, "foo", 1))',
                            },
                        },
                    },
                ],
                ...
                mockRPC: function (route, { method, model, args }) {
                    if (model === "documents.document" && method === "write") {
                        ...
                        assert.deepEqual(
                            ...
                            '=LEEWISE.PIVOT(1,"probability","foo","1")'
                        );
                    }
                },
            });
        ...
        QUnit.test("Leewise locales are displayed in setting side panel", async function (assert) {
            const { env } = await createSpreadsheet({
