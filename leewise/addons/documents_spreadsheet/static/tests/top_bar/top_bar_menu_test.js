...
QUnit.module("documents_spreadsheet > Topbar Menu Items", {}, function () {
    ...
    QUnit.test("Can Insert leewise formulas from Insert > Functions > Leewise", async function (assert) {
        ...
        await click(document.querySelector(".o-menu-root[title='Leewise']"));
        await click(document.querySelector(".o-menu-item[title='LEEWISE.CURRENCY.RATE']"));

        await triggerEvent(document.activeElement, null, "keydown", { key: "Enter" });

        assert.equal(getCellContent(model, "A1"), "=LEEWISE.CURRENCY.RATE()");
    });
});
