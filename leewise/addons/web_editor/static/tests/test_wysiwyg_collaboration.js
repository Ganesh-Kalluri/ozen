...
import {
    ...
    patchEditorIframe,
} from '@web_editor/js/editor/leewise-editor/test/utils';
...
class PeerTest {
    ...
    async startEditor() {
        ...
        if (this.initialParsedSelection) {
            ...
            this.wysiwyg.leewiseEditor._recordHistorySelection();
        } else {
    ...
    async getValue() {
        this.wysiwyg.leewiseEditor.observerUnactive('PeerTest.getValue');
        ...
        this.wysiwyg.leewiseEditor.observerActive('PeerTest.getValue');
...
export async function createPeers(peers) {
    ...
    for (const peerId of peers) {
        ...
        patch(wysiwyg, {
            ...
            // Hacky hook as we know this method is called after setting the value in the wysiwyg start and before sending the value to leewiseEditor.
    ...
    QUnit.module('Collaboration', {}, () => {
        ...
        QUnit.module('Focus', {}, () => {
            QUnit.test('Focused client should not receive step if no data channel is open', async (assert) => {
                ...
                await peers.p1.wysiwyg.leewiseEditor.execCommand('insert', 'b');
        ...
        QUnit.module('Stale detection & recovery', {}, () => {
            QUnit.module('detect stale while unfocused', async () => {
                QUnit.test('should do nothing until focus', async (assert) => {
                    ...
                    await peers.p1.wysiwyg.leewiseEditor.execCommand('insert', 'c');
        ...
        QUnit.module('Disconnect & reconnect', {}, () => {
            QUnit.test('should sync history when disconnecting and reconnecting to internet', async (assert) => {
                ...
                const setSelection = peer => {
                    ...
                    const pElement = peer.wysiwyg.leewiseEditor.editable.querySelector('p')
                ...
                const addP = (peer, content) => {
                    ...
                    peer.wysiwyg.leewiseEditor.editable.append(p);
                    peer.wysiwyg.leewiseEditor.historyStep();
                }
                ...
                await peers.p2.wysiwyg.leewiseEditor.execCommand('insert', 'e');
                addP(peers.p2, 'f');
