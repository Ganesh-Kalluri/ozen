...
const resetOuids = LeewiseEditorLib.resetOuids;
let _serviceCache = {
    orm: {},
    rpc: {},
};
...
const SnippetOptionWidget = Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.leewiseEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.leewiseEditor.automaticStepActive('preview_option');
        }
    ...
    _onUserValueUpdate: async function (ev) {
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.leewiseEditor.unbreakableStepUnactive();
        }
        this.trigger_up('snippet_edition_request', {exec: async () => {
            ...
            if (shouldRecordUndo) {
                this.options.wysiwyg.leewiseEditor.historyStep();
            }
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ...
            if (rowEl.classList.contains("o_grid_mode") && !isMobile) {
                self.options.wysiwyg.leewiseEditor.observerUnactive('displayBackgroundGrid');
                backgroundGridEl = gridUtils._addBackgroundGrid(rowEl, 0);
                gridUtils._setElementToMaxZindex(backgroundGridEl, rowEl);
                self.options.wysiwyg.leewiseEditor.observerActive('displayBackgroundGrid');
            }
            ...
            self.options.wysiwyg.leewiseEditor.automaticStepUnactive('resizing');
            ...
            const bodyMouseUp = function () {
                ...
                self.options.wysiwyg.leewiseEditor.automaticStepActive('resizing');
                ...
                setTimeout(function () {
                    self.options.wysiwyg.leewiseEditor.historyStep();
...
registry.layout_column = SnippetOptionWidget.extend(ColumnLayoutMixin, {
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["row-gap", "column-gap"].includes(params.cssProperty) && !isMobileView) {
            ...
            this.options.wysiwyg.leewiseEditor.observerUnactive("addGridPreview");
            ...
            this.options.wysiwyg.leewiseEditor.observerActive("addGridPreview");
     ...
     */
    _removeGridPreview() {
        this.options.wysiwyg.leewiseEditor.observerUnactive("removeGridPreview");
        ...
        this.options.wysiwyg.leewiseEditor.observerActive("removeGridPreview");
    },
    /**
     * @returns {boolean}
     */
...
registry.GridColumns = SnippetOptionWidget.extend({
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["--grid-item-padding-y", "--grid-item-padding-x"].includes(params.cssProperty)) {
            ...
            this.options.wysiwyg.leewiseEditor.observerUnactive("addPaddingPreview");
            this.$target[0].classList.add("o_we_padding_highlight");
            this.options.wysiwyg.leewiseEditor.observerActive("addPaddingPreview");
    ...
    _removePaddingPreview() {
        this.options.wysiwyg.leewiseEditor.observerUnactive("removePaddingPreview");
        ...
        this.options.wysiwyg.leewiseEditor.observerActive("removePaddingPreview");
    },
});
...
registry.ReplaceMedia = SnippetOptionWidget.extend({
     ...
     */
    onFocus() {
        this.options.wysiwyg.leewiseEditor.addEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.leewiseEditor.addEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
     ...
     */
    onBlur() {
        this.options.wysiwyg.leewiseEditor.removeEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.leewiseEditor.removeEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
    },
...
registry.ImageTools = ImageHandlerOption.extend({
    ...
    async setImgShapeHoverEffect(previewMode, widgetValue, params) {
        ...
        if (this.firstHoverEffect) {
            this.options.wysiwyg.leewiseEditor.historyUnpauseSteps();
...
￼    */
    ...
    async _writeShape(svgText) {
        ...
        if (hasHoverEffect) {
            ...
            this.options.wysiwyg.leewiseEditor.observerUnactive("addClonedImgForHoverEffectPreview");
            img.classList.add("d-none");
            img.insertAdjacentElement("afterend", clonedImgEl);
            this.options.wysiwyg.leewiseEditor.observerActive("addClonedImgForHoverEffectPreview");
        }
        const loadedImg = await loadImage(dataURL, img);
        if (hasHoverEffect) {
            this.options.wysiwyg.leewiseEditor.observerUnactive("removeClonedImgForHoverEffectPreview");
            clonedImgEl.remove();
            img.classList.remove("d-none");
            this.options.wysiwyg.leewiseEditor.observerActive("removeClonedImgForHoverEffectPreview");
        }
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Leewise).
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.leewiseEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.leewiseEditor.observerActive('_markColorLevel');
    },
});
...
registry.VersionControl = SnippetOptionWidget.extend({
    ...
    async replaceSnippet() {
        ...
        // Replacing the block.
        this.options.wysiwyg.leewiseEditor.historyPauseSteps();
