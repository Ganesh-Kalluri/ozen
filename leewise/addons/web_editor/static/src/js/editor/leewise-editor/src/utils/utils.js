...
export function closestElement(node, predicate = "*") {
    ...
    return element?.closest('.leewise-editor-editable') && element;
}
...
export function getNormalizedCursorPosition(node, offset, full = true) {
    const editable = closestElement(node, '.leewise-editor-editable');
...
// TODO rename this function in master: it also handles Leewise icons, not only
...
export function prepareUpdate(...args) {
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, '.leewise-editor-editable');
