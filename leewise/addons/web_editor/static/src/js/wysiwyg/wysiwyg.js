...
import * as LeewiseEditorLib from "@web_editor/js/editor/leewise-editor/src/LeewiseEditor";
...
import { isSelectionInSelectors, peek } from '@web_editor/js/editor/leewise-editor/src/utils/utils';
...
const LeewiseEditor = LeewiseEditorLib.LeewiseEditor;
const getDeepRange = LeewiseEditorLib.getDeepRange;
const getInSelection = LeewiseEditorLib.getInSelection;
const isProtected = LeewiseEditorLib.isProtected;
const rgbToHex = LeewiseEditorLib.rgbToHex;
const preserveCursor = LeewiseEditorLib.preserveCursor;
const closestElement = LeewiseEditorLib.closestElement;
const setSelection = LeewiseEditorLib.setSelection;
const endPos = LeewiseEditorLib.endPos;
const hasValidSelection = LeewiseEditorLib.hasValidSelection;
const parseHTML = LeewiseEditorLib.parseHTML;
const closestBlock = LeewiseEditorLib.closestBlock;
const getRangePosition = LeewiseEditorLib.getRangePosition;
const getCursorDirection = LeewiseEditorLib.getCursorDirection;
const DIRECTIONS = LeewiseEditorLib.DIRECTIONS;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.leewiseEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.leewiseEditor.historyUnstash();
                ...
                // hides it).
                if (this.leewiseEditor.deselectTable() && hasValidSelection(this.leewiseEditor.editable)) {
                    this.leewiseEditor.document.getSelection().collapseToStart();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.leewiseEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                } finally {
                    this.leewiseEditor.historyUnpauseSteps();
                }
            }
        };
    ...
    async startEdition() {
        ...
        this.leewiseEditor = new LeewiseEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.leewiseEditor.addEventListener('contentChanged', function () {
        ...
        if (this.leewiseEditor.document !== document) {
            this.leewiseEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.leewiseEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.leewiseEditor.document.addEventListener("keyup", this._signalOnline, true);
        }

        this._initialValue = this.getValue();
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.leewiseEditor.multiselectionRefresh, { passive: true });
        ...
        this._observeLeewiseFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.leewiseEditor.observerUnactive();
                const $field = $(this);
                ...
                self.leewiseEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.leewiseEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            $(this.leewiseEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.leewiseEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.leewiseEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.leewiseEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.leewiseEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.leewiseEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        this.setCSSVariables(this.snippetsMenu ? this.snippetsMenu.el : this.toolbarEl);

        this.leewiseEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.leewiseEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.leewiseEditor.isMobile) {
                this.leewiseEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.leewiseEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.leewiseEditor) {
            this.leewiseEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.leewiseEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.leewiseEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.leewiseEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.leewiseEditor.destroy();
        }
        if (this.snippetsMenu) {
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length && this.leewiseEditor) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.leewiseEditor.multiselectionRefresh, { passive: true });
        }
        $(this.$root).off('click');
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper leewise-editor">');
            this.$root = $wrapper;
    ...
    getValue(options) {
        ...
        this.leewiseEditor && this.leewiseEditor.cleanForSave($editable[0]);
     ...
     */
    historyReset() {
        this.leewiseEditor.historyReset();
    }
    /**
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.leewiseEditor) : [];
     ...
     */
    setValue(value) {
        this.leewiseEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.leewiseEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.leewiseEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.leewiseEditor && !this.leewiseEditor.historyResetLatestComputedSelection(true)) {
    ...
    getDeepRange() {
        return getDeepRange(this.leewiseEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.leewiseEditor.editable) {
        if (this.leewiseEditor) {
            this.leewiseEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.leewiseEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the Leewise field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeLeewiseFieldChanges() {
        ...
        if (this.leewiseFieldObservers) {
            for (let observerData of this.leewiseFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const leewiseFieldSelector = '[data-oe-model], [data-oe-translation-initial-sha]';
            const $leewiseFields = this.$editable.find(leewiseFieldSelector);
            this.leewiseFieldObservers = [];

            $leewiseFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.leewiseEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $leewiseFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    this._pauseLeewiseFieldObservers();
                    ...
                    this.leewiseEditor.withoutRollback(() => {
                ...
                this.leewiseFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.leewiseEditor.document.getSelection().getRangeAt(0).startContainer;
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.leewiseEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.leewiseEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.leewiseEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.leewiseEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.leewiseEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.leewiseEditor.editable,
                        link,
                ...
                const _onClick = ev => {
                    if (
                        ...
                        this.destroyLinkTools();
                        this.leewiseEditor.document.removeEventListener('click', _onClick, true);
                        document.removeEventListener('click', _onClick, true);
                    }
                };
                this.leewiseEditor.document.addEventListener('click', _onClick, true);
                document.addEventListener('click', _onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.leewiseEditor.historySize() - 1;
            this.leewiseEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onSave: (data) => {
                    ...
                    this.leewiseEditor.historyUnpauseSteps();
                    this.leewiseEditor.historyStep();
                ...
                onClose: () => {
                    this.leewiseEditor.historyUnpauseSteps();
                    this.leewiseEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt') {
        const restore = preserveCursor(this.leewiseEditor.document);
        const params = {
            insert: content => {
                this.leewiseEditor.historyPauseSteps();
                const insertedNodes = this.leewiseEditor.execCommand('insert', content);
                this.leewiseEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.leewiseEditor.editable.parentElement;
        ...
        if (mode === 'alternatives') {
            params.originalText = this.leewiseEditor.document.getSelection().toString() || '';
        }
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.snippetsMenu._mutex.exec(() => {
                // Wait for the editor panel to be fully updated.
                this.leewiseEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.leewiseEditor.execCommand('unlink');
        }
    }
    /**
    ...
    showImageFullscreen(url) {
        ...
        this.leewiseEditor.document.getSelection()?.collapseToEnd();
        this.leewiseEditor.editable.blur();
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.leewiseEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.leewiseEditor.document);

        const editable = LeewiseEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.leewiseEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.leewiseEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.leewiseEditor.execCommand('insert', str);
                },
            }, {
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.leewiseEditor.unbreakableStepUnactive();
            this.leewiseEditor.historyStep();
        } else {
            return this.leewiseEditor.execCommand('insert', element);
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.leewiseEditor.document, selector);
    }
    /**
    ...
    async _createSnippetsMenuInstance(options={}) {
        const snippetsEditor = await leewise.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.leewiseEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.leewiseEditor.document});
            const mouseup = () => {
                $('#image-transform').toggleClass('active', $image.is('[style*="transform"]'));
            };
            $(this.leewiseEditor.document).on('mouseup', mouseup);
            const mousedown = mousedownEvent => {
                if (!$(mousedownEvent.target).closest('.transfo-container').length) {
                    $image.transfo('destroy');
                    $(this.leewiseEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                }
            ...
            $(this.leewiseEditor.document).on('mousedown', mousedown);
        });
    ...
    _showImageCrop() {
        ...
        this.leewiseEditor.toolbarHide();
        $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.leewiseEditor.toolbarShow());
    }
    /**
     * @private
     * @param {jQuery} $
    ...
    onColorpaletteDropdownShow(colorType) {
        ...
        this.leewiseEditor.historyStash();
    }
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        let coloredElements = this.leewiseEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.leewiseEditor.document.contains(element));
        ...
        if (coloredTds.length) {
            ...
            sel.addRange(getDeepRange(this.leewiseEditor.editable, { range }));
        }

        const hexColor = this._colorToHex(color);
        this.leewiseEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.leewiseEditor.document.getSelection();
        if (!selection) return;
        ...
        this.leewiseEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.leewiseEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.leewiseEditor.document.getSelection();
            const range = this.leewiseEditor.document.createRange();
        ...
        this.leewiseEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent LeewiseEditor from controlling it as this is
    ...
    _getBannerCommand(title, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.leewiseEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.leewiseEditor.document, `
                ...
                `).childNodes[0];
                this.leewiseEditor.execCommand('insert', bannerElement);
                this.leewiseEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...
            this._getBannerCommand(_t('Banner Danger'), 'danger', 'fa-exclamation-circle', _t('Insert a danger banner'), 21),
            {
                ...
                isDisabled: () => !this.leewiseEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.leewiseEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.leewiseEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.leewiseEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (user && user.signature) {
                        this.leewiseEditor.execCommand('insert', parseHTML(this.leewiseEditor.document, user.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.leewiseEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.leewiseEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.leewiseEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.leewiseEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.leewiseEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.leewiseEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.leewiseEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.leewiseEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.leewiseEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from leewise field value
        // ZeroWidthSpace may be present from LeewiseEditor edition process
    ...
    _onSelectionChange() {
        if (this.leewiseEditor.autohideToolbar && this.linkPopover) {
            const selectionInLink = getInSelection(this.leewiseEditor.document, 'a') === this.linkPopover.target;
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.leewiseEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.leewiseEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.leewiseEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.leewiseEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.leewiseEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.leewiseEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.leewiseEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.leewiseEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.leewiseEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.leewiseEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.leewiseEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.leewiseEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.leewiseEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.leewiseEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.leewiseEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the leewise bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.leewiseEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.leewiseEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.leewiseEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.leewiseEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.leewiseEditor.historyResetFromSteps(steps, historyIds);
        this.leewiseEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.leewiseEditor && !this.leewiseEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.leewiseEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.leewiseEditor.historyGetBranchIds().join(',');
