...
import { Model, DispatchResult, helpers, tokenize } from "@leewise/o-spreadsheet";
import {
...
QUnit.module("spreadsheet > Global filters model", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const chartId = model.getters.getLeewiseChartIds()[0];
            fieldMatching = model.getters.getLeewiseChartFieldMatching(chartId, filterId);
            assert.equal(fieldMatching.chain, "created_on");
            assert.equal(fieldMatching.type, "date");
            assert.equal(fieldMatching.offset, undefined);
        }
    ...
    QUnit.test("LEEWISE.FILTER.VALUE text filter", async function (assert) {
        assert.expect(3);

        const model = await createModelWithDataSource();
        setCellContent(model, "A10", `=LEEWISE.FILTER.VALUE("Text Filter")`);
        await nextTick();
    ...
    QUnit.test("LEEWISE.FILTER.VALUE date filter", async function (assert) {
        assert.expect(4);

        const model = await createModelWithDataSource();
        setCellContent(model, "A10", `=LEEWISE.FILTER.VALUE("Date Filter")`);
        await nextTick();
        await addGlobalFilter(model, {
            id: "42",
            type: "date",
    ...
    QUnit.test("LEEWISE.FILTER.VALUE date from/to without values", async function (assert) {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=LEEWISE.FILTER.VALUE("Date Filter")`);
        await addGlobalFilter(model, {
            id: "42",
            type: "date",
            label: "Date Filter",
    ...
    QUnit.test("LEEWISE.FILTER.VALUE date from/to with only from defined", async function (assert) {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=LEEWISE.FILTER.VALUE("Date Filter")`);
        await addGlobalFilter(model, {
            id: "42",
    ...
    QUnit.test("LEEWISE.FILTER.VALUE date from/to with only to defined", async function (assert) {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=LEEWISE.FILTER.VALUE("Date Filter")`);
        await addGlobalFilter(model, {
            id: "42",
    ...
    QUnit.test("LEEWISE.FILTER.VALUE date from/to with from and to defined", async function (assert) {
        const model = await createModelWithDataSource();
        setCellContent(model, "A1", `=LEEWISE.FILTER.VALUE("Date Filter")`);
        await addGlobalFilter(model, {
            id: "42",
    ...
    QUnit.test("LEEWISE.FILTER.VALUE relation filter", async function (assert) {
        const model = await createModelWithDataSource({
        ...
        setCellContent(model, "A10", `=LEEWISE.FILTER.VALUE("Relation Filter")`);
        await nextTick();
        await addGlobalFilter(model, {
            id: "42",
            type: "relation",
    ...
    QUnit.test(
        "LEEWISE.FILTER.VALUE with escaped quotes in the filter label",
        async function (assert) {
            const model = await createModelWithDataSource();
            await addGlobalFilter(model, {
                id: "42",
            ...
            setCellContent(model, "A1", '=LEEWISE.FILTER.VALUE("my \\"special\\" filter")');
            assert.equal(getCellValue(model, "A1"), "Jean-Jacques");
        }
    );

    QUnit.test(
        "LEEWISE.FILTER.VALUE formulas are updated when filter label is changed",
        async function (assert) {
            assert.expect(1);
            ...
            setCellContent(
                model,
                "A10",
                `=LEEWISE.FILTER.VALUE("Cuillère") & LEEWISE.FILTER.VALUE( "Cuillère" )`
            );
            const [filter] = model.getters.getGlobalFilters();
            const newFilter = {
                ...filter,
            ...
            assert.equal(
                ...
                `=LEEWISE.FILTER.VALUE("Interprete") & LEEWISE.FILTER.VALUE("Interprete")`
            );
        }
    );
                ...
                sheets: [
                    {
                        id: "sheet1",
                        cells: {
                            A1: { content: `=LEEWISE.PIVOT("1", "probability")` },
                        },
                    },
                ],
                pivots: {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.verifySteps([]);
            setCellContent(model, "A1", `=LEEWISE.PIVOT("1", "probability")`);
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const dateFilters2 = getFiltersMatchingPivot(
                model,
                '=LEEWISE.PIVOT.HEADER(1,"date:year","2016")'
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const filters = getFiltersMatchingPivot(
                model,
                '=LEEWISE.PIVOT.HEADER(1,"date:month","08/2016","product_id","41")'
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=LEEWISE.PIVOT(1, "__count", "#product_id", 1)');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=LEEWISE.PIVOT(1, "probability", "#product_id", 1)');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=LEEWISE.PIVOT(1, "probability")');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test("field matching is removed when list is deleted", async function (assert) {
        ...
        model.dispatch("REMOVE_LEEWISE_LIST", { listId });
        assert.throws(
    ...
    QUnit.test("field matching is removed when an Leewise chart is deleted", async function (assert) {
        const { model } = await createSpreadsheetWithChart({ type: "leewise_pie" });
        const sheetId = model.getters.getActiveSheetId();
        ...
        assert.deepEqual(model.getters.getLeewiseChartFieldMatching(chartId, filter.id), matching);
        model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
        assert.throws(
            () => model.getters.getLeewiseChartFieldMatching(chartId, filter.id),
