...
import { constants, tokenize } from "@leewise/o-spreadsheet";
...
QUnit.module("spreadsheet > pivot_helpers", {}, () => {
    QUnit.test("Basic formula extractor", async function (assert) {
        const formula = `=LEEWISE.PIVOT("1", "test") + LEEWISE.LIST("2", "hello", "bla")`;
        const tokens = tokenize(formula);
        let functionName;
        let args;
        ({ functionName, args } = getFirstPivotFunction(tokens));
        assert.strictEqual(functionName, "LEEWISE.PIVOT");
        ...
        assert.strictEqual(functionName, "LEEWISE.LIST");
    ...
    QUnit.test("Extraction with two PIVOT formulas", async function (assert) {
        const formula = `=LEEWISE.PIVOT("1", "test") + LEEWISE.PIVOT("2", "hello", "bla")`;
        const tokens = tokenize(formula);
    ...
    QUnit.test("Number of formulas", async function (assert) {
        const formula = `=LEEWISE.PIVOT("1", "test") + LEEWISE.PIVOT("2", "hello", "bla") + LEEWISE.LIST("1", "bla")`;
