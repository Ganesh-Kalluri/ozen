...
QUnit.module("spreadsheet > pivot plugin", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithPivot();
            setCellContent(model, "C3", `=LEEWISE.PIVOT(G10,"probability","bar","false","foo","2")+2`);
    ...
    QUnit.test("Can remove a pivot with undo after editing a cell", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.ok(getCellContent(model, "B1").startsWith("=LEEWISE.PIVOT.HEADER"));
    ...
    QUnit.test("rename pivot with empty name is refused", async (assert) => {
        const { model } = await createSpreadsheetWithPivot();
        const result = model.dispatch("RENAME_LEEWISE_PIVOT", {
            pivotId: "1",
            name: "",
        });
    ...
    QUnit.test("Undo/Redo for RENAME_LEEWISE_PIVOT", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.equal(model.getters.getPivotName("1"), "Partner Pivot");
        model.dispatch("RENAME_LEEWISE_PIVOT", { pivotId: "1", name: "test" });
    ...
    QUnit.test("Format header displays an error for non-existing field", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "G10", `=LEEWISE.PIVOT.HEADER("1", "measure", "non-existing")`);
        setCellContent(model, "G11", `=LEEWISE.PIVOT.HEADER("1", "non-existing", "bla")`);
        await nextTick();
            ...
            sheets: [
                {
                    ...
                    cells: {
                        A1: { content: `=LEEWISE.PIVOT(1, "probability")` },
                        A2: { content: `=LEEWISE.PIVOT(2, "probability")` },
                    },
                },
            ],
            pivots: {
    ...
    QUnit.test("evaluates only once when two pivots are loading", async function (assert) {
        ...
        setCellContent(model, "A1", '=LEEWISE.PIVOT("1", "probability")');
        setCellContent(model, "A2", '=LEEWISE.PIVOT("2", "probability")');
        assert.equal(getCellValue(model, "A1"), "Loading...");
            ...
            sheets: [
                {
                    id: "sheet1",
                    cells: {
                        A1: { content: `=LEEWISE.PIVOT.HEADER(1, "measure", "probability")` },
                        A2: { content: `=LEEWISE.PIVOT.HEADER(1, "product_id", 37)` },
                        A3: { content: `=LEEWISE.PIVOT(1, "probability")` },
                    },
                },
            ],
            pivots: {
    ...
    QUnit.test("pivot grouped by char field which represents numbers", async function (assert) {
        ...
        assert.strictEqual(getCell(model, "A3").content, '=LEEWISE.PIVOT.HEADER(1,"name","000111")');
        assert.strictEqual(getCell(model, "A4").content, '=LEEWISE.PIVOT.HEADER(1,"name",111)');
        ...
        assert.strictEqual(
            ...
            '=LEEWISE.PIVOT(1,"probability","name","000111")'
        );
        assert.strictEqual(getCell(model, "B4").content, '=LEEWISE.PIVOT(1,"probability","name",111)');
    ...
    QUnit.test("relational PIVOT.HEADER with missing id", async function (assert) {
        ...
        model.dispatch("UPDATE_CELL", {
            col: 4,
            row: 9,
            content: `=LEEWISE.PIVOT.HEADER("1", "product_id", "1111111")`,
            sheetId,
        });
        await waitForDataSourcesLoaded(model);
        assert.equal(
    ...
    QUnit.test("relational PIVOT.HEADER with undefined id", async function (assert) {
        ...
        setCellContent(model, "F10", `=LEEWISE.PIVOT.HEADER("1", "product_id", A25)`);
    ...
    QUnit.test("can import (export) contextual domain", async (assert) => {
        ...
        setCellContent(model, "A1", '=LEEWISE.PIVOT(1, "probability")'); // load the data (and check the rpc domain)
        await nextTick();
        assert.strictEqual(
    ...
    QUnit.test("Can group by many2many field ", async (assert) => {
        ...
        assert.equal(getCellFormula(model, "A3"), '=LEEWISE.PIVOT.HEADER(1,"tag_ids","false")');
        assert.equal(getCellFormula(model, "A4"), '=LEEWISE.PIVOT.HEADER(1,"tag_ids",42)');
        assert.equal(getCellFormula(model, "A5"), '=LEEWISE.PIVOT.HEADER(1,"tag_ids",67)');

        assert.equal(
            getCellFormula(model, "B3"),
            '=LEEWISE.PIVOT(1,"probability","tag_ids","false","foo",1)'
        );
        assert.equal(
            getCellFormula(model, "B4"),
            '=LEEWISE.PIVOT(1,"probability","tag_ids",42,"foo",1)'
        );
        assert.equal(
            getCellFormula(model, "B5"),
            '=LEEWISE.PIVOT(1,"probability","tag_ids",67,"foo",1)'
        );

        assert.equal(
            getCellFormula(model, "C3"),
            '=LEEWISE.PIVOT(1,"probability","tag_ids","false","foo",2)'
        );
        assert.equal(
            getCellFormula(model, "C4"),
            '=LEEWISE.PIVOT(1,"probability","tag_ids",42,"foo",2)'
        );
        assert.equal(
            getCellFormula(model, "C5"),
            '=LEEWISE.PIVOT(1,"probability","tag_ids",67,"foo",2)'
        );
    ...
    QUnit.test("can edit pivot domain", async (assert) => {
        ...
        model.dispatch("UPDATE_LEEWISE_PIVOT_DOMAIN", {
            pivotId,
            domain: [["foo", "in", [55]]],
        });
