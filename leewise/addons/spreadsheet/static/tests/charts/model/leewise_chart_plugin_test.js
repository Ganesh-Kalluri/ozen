...
import { LeewiseBarChart } from "@spreadsheet/chart/leewise_chart/leewise_bar_chart";
import { LeewiseChart } from "@spreadsheet/chart/leewise_chart/leewise_chart";
import { LeewiseLineChart } from "@spreadsheet/chart/leewise_chart/leewise_line_chart";
...
QUnit.module("spreadsheet > leewise chart plugin", {}, () => {
    QUnit.test("Can add an Leewise Bar chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "leewise_bar" });
        const sheetId = model.getters.getActiveSheetId();
        assert.strictEqual(model.getters.getChartIds(sheetId).length, 1);
        const chartId = model.getters.getChartIds(sheetId)[0];
        const chart = model.getters.getChart(chartId);
        assert.ok(chart instanceof LeewiseBarChart);
    ...
    QUnit.test("Can add an Leewise Line chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "leewise_line" });
        const sheetId = model.getters.getActiveSheetId();
        assert.strictEqual(model.getters.getChartIds(sheetId).length, 1);
        const chartId = model.getters.getChartIds(sheetId)[0];
        const chart = model.getters.getChart(chartId);
        assert.ok(chart instanceof LeewiseLineChart);
        assert.strictEqual(chart.getDefinitionForExcel(), undefined);
        assert.strictEqual(model.getters.getChartRuntime(chartId).chartJsConfig.type, "line");
    });

    QUnit.test("Can add an Leewise Pie chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "leewise_pie" });
        const sheetId = model.getters.getActiveSheetId();
        assert.strictEqual(model.getters.getChartIds(sheetId).length, 1);
        const chartId = model.getters.getChartIds(sheetId)[0];
        const chart = model.getters.getChart(chartId);
        assert.ok(chart instanceof LeewiseChart);
        assert.strictEqual(chart.getDefinitionForExcel(), undefined);
    ...
    QUnit.test("Leewise bar chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "leewise_bar",
    ...
    QUnit.test("Leewise pie chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "leewise_pie",
            mockRPC: async function (route, args) {
                if (args.method === "web_read_group") {
    ...
    QUnit.test("Leewise line chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "leewise_line",
            mockRPC: async function (route, args) {
                if (args.method === "web_read_group") {
    ...
    QUnit.test("Can import/export an Leewise chart", async (assert) => {
        const model = await createModelWithDataSource();
        insertChartInSpreadsheet(model, "leewise_line");
        const data = model.exportData();
        const figures = data.sheets[0].figures;
        assert.strictEqual(figures.length, 1);
        const figure = figures[0];
        assert.strictEqual(figure.tag, "chart");
        assert.strictEqual(figure.data.type, "leewise_line");
    ...
    QUnit.test("Can undo/redo an Leewise chart creation", async (assert) => {
        const model = await createModelWithDataSource();
        insertChartInSpreadsheet(model, "leewise_line");
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    QUnit.test("charts with no legend", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "leewise_pie" });
        insertChartInSpreadsheet(model, "leewise_bar");
        insertChartInSpreadsheet(model, "leewise_line");
        const sheetId = model.getters.getActiveSheetId();
    ...
    QUnit.test("Can copy/paste Leewise chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "leewise_pie" });
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
        ...
        assert.ok(model.getters.getChart(chartIds[1]) instanceof LeewiseChart);
        assert.strictEqual(
    ...
    QUnit.test("Can cut/paste Leewise chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "leewise_pie" });
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
        ...
        assert.ok(model.getters.getChart(chartIds[0]) instanceof LeewiseChart);
        assert.strictEqual(
    ...
    QUnit.test("Duplicating a sheet correctly duplicates Leewise chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "leewise_bar" });
        const sheetId = model.getters.getActiveSheetId();
    ...
    QUnit.test(
        "Load leewise chart spreadsheet with models that cannot be accessed",
    ...
    QUnit.test("Can insert leewise chart from a different model", async (assert) => {
        const model = await createModelWithDataSource();
