    ...
    () => {
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.equal(
                    exportedData.chartLeewiseMenusReferences[chartId],
                    1,
                    "Link to leewise menu is exported"
                );
                const importedModel = new Model(exportedData, { custom: { env } });
                const chartMenu = importedModel.getters.getChartLeewiseMenu(chartId);
                assert.equal(chartMenu.id, 1, "Link to leewise menu is imported");
            }
        );

        QUnit.test("Can undo-redo a LINK_LEEWISE_MENU_TO_CHART", async function (assert) {
            const env = await makeTestEnv({ serverData: this.serverData });
            const model = new Model({}, { custom: { env } });
            createBasicChart(model, chartId);
            model.dispatch("LINK_LEEWISE_MENU_TO_CHART", {
            ...
            assert.equal(model.getters.getChartLeewiseMenu(chartId).id, 1);
            model.dispatch("REQUEST_UNDO");
            assert.equal(model.getters.getChartLeewiseMenu(chartId), undefined);
