...
QUnit.module(
    ...
    () => {
        QUnit.test(
            "icon external link isn't on the chart when its not linked to an leewise menu",
            async function (assert) {
                ...
                await nextTick();
                const leewiseMenu = model.getters.getChartLeewiseMenu(chartId);
                assert.equal(leewiseMenu, undefined, "No menu linked with the chart");
        ...
        QUnit.test(
            "icon external link is on the chart when its linked to an leewise menu",
            async function (assert) {
                const model = await createModelWithDataSource({
                    serverData: this.serverData,
                });
                const fixture = await mountSpreadsheet(model);
                createBasicChart(model, chartId);
                model.dispatch("LINK_LEEWISE_MENU_TO_CHART", {
                    chartId,
                    leewiseMenuId: 1,
                });
                const chartMenu = model.getters.getChartLeewiseMenu(chartId);
                assert.equal(chartMenu.id, 1, "Leewise menu is linked to chart");
                await nextTick();
                const externalRefIcon = fixture.querySelector(".o-chart-external-link");
        ...
        QUnit.test(
            "icon external link is not on the chart when its linked to a wrong leewise menu",
            async function (assert) {
                const model = await createModelWithDataSource({
                    serverData: this.serverData,
                });
                ...
                model.dispatch("LINK_LEEWISE_MENU_TO_CHART", {
                    chartId,
                    leewiseMenuId: "menu which does not exist",
                });
                const chartMenu = model.getters.getChartLeewiseMenu(chartId);
        ...
        QUnit.test(
            "click on icon external link on chart redirect to the leewise menu",
            async function (assert) {
                ...
                const chartMenu = model.getters.getChartLeewiseMenu(chartId);
                assert.equal(chartMenu.id, 2, "Leewise menu is linked to chart");
                await nextTick();
        ...
        QUnit.test(
            "Click on chart in dashboard mode redirect to the leewise menu",
            async function (assert) {
                const doActionStep = "doAction";
                mockActionService(assert, doActionStep);
                const model = await createModelWithDataSource({
                ...
                assert.verifySteps(
                    [doActionStep],
                    "Clicking on a chart while on dashboard mode redirect to the leewise menu"
                );
            }
        );
        ...
        QUnit.test("can use menus xmlIds instead of menu ids", async function (assert) {
            ...
            model.dispatch("LINK_LEEWISE_MENU_TO_CHART", {
                chartId,
                leewiseMenuId: "documents_spreadsheet.test.menu2",
            });
            await nextTick();
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                model.dispatch("LINK_LEEWISE_MENU_TO_CHART", {
                    chartId,
                    leewiseMenuId: "documents_spreadsheet.test.menu_without_action",
                });
                await nextTick();
