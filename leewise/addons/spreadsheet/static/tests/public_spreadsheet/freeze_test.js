...
import { freezeLeewiseData } from "../../src/helpers/model";
...
QUnit.module("freezing spreadsheet", {}, function () {
    QUnit.test("leewise pivot functions are replaced with their value", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.strictEqual(getCell(model, "A3").content, '=LEEWISE.PIVOT.HEADER(1,"bar","false")');
        assert.strictEqual(
        ...
        const data = await freezeLeewiseData(model);
    ...
    QUnit.test("invalid expression with pivot function", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "A1", "=LEEWISE.PIVOT(1)+"); // invalid expression
    ...
    QUnit.test("leewise pivot functions detection is not case sensitive", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "A1", '=leewise.pivot(1,"probability")');
        setCellContent(model, "A2", '=LEEWISE.pivot(1,"probability")');
        const data = await freezeLeewiseData(model);
    ...
    QUnit.test("computed format is exported", async function (assert) {
        ...
        setCellContent(model, "A1", '=LEEWISE.PIVOT(1,"pognon")');
    ...
    QUnit.test("leewise charts are replaced with an image", async function (assert) {
