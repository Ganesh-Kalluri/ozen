...
import { migrate, LEEWISE_VERSION } from "@spreadsheet/o_spreadsheet/migration";
import { Model } from "@leewise/o-spreadsheet";

QUnit.module("spreadsheet > migrations");

QUnit.test("Leewise formulas are migrated", (assert) => {
    const data = {
        sheets: [
            {
                cells: {
    ...
    assert.strictEqual(migratedData.sheets[0].cells.A1.content, `=LEEWISE.PIVOT("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A2.content, `=LEEWISE.PIVOT.HEADER("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=LEEWISE.FILTER.VALUE("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=LEEWISE.LIST("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A5.content, `=LEEWISE.LIST.HEADER("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A6.content, `=LEEWISE.PIVOT.POSITION("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A7.content, `=LEEWISE.PIVOT("1")`);
});
...
QUnit.test("Pivot 'day' arguments are migrated", (assert) => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=LEEWISE.PIVOT("1","21/07/2022")` },
                    A2: { content: `=LEEWISE.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=leewise.pivot("1","21/07/2021")` },
                    A4: { content: `=LEEWISE.PIVOT("1","test")` },
                    A5: { content: `=leewise.pivot("1","21/07/2021")+"21/07/2021"` },
    ...
    const migratedData = migrate(data);
    assert.strictEqual(migratedData.sheets[0].cells.A1.content, `=LEEWISE.PIVOT("1","07/21/2022")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A2.content,
        `=LEEWISE.PIVOT.HEADER("1","12/11/2022")`
    );
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=leewise.pivot("1","07/21/2021")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=LEEWISE.PIVOT("1","test")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A5.content,
        `=leewise.pivot("1","07/21/2021")+"21/07/2021"`
    );
...
QUnit.test("Leewise version is exported", (assert) => {
    const model = new Model();
    assert.strictEqual(model.exportData().leewiseVersion, LEEWISE_VERSION);
});
