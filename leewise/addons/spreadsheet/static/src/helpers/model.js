...
import { Model, parse, helpers, iterateAstNodes } from "@leewise/o-spreadsheet";
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const figure of sheet.figures) {
            if (figure.tag === "chart" && figure.data.type.startsWith("leewise_")) {
                ...
                const img = leewiseChartToImage(model, figure);
                figure.tag = "image";
                figure.data = {
                    path: img,
 ...
 * @returns {boolean}
 */
function containsLeewiseFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
        (!content.toUpperCase().includes("LEEWISE.") && !content.toUpperCase().includes("_T"))
    ) {
        return false;
    }
    try {
        ...
        return iterateAstNodes(ast).some(
            (ast) =>
                ast.type === "FUNCALL" &&
                (ast.value.toUpperCase().startsWith("LEEWISE.") ||
 ...
 * @returns {string}
 */
function leewiseChartToImage(model, figure) {
