...
export class PivotUIPlugin extends spreadsheet.UIPlugin {
    ...
    handleEvent(event) {
        ...
        switch (event.type) {
            case "ZonesSelected": {
                ...
                if (cell !== undefined && cell.content.startsWith("=LEEWISE.PIVOT.HEADER(")) {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "REMOVE_PIVOT":
                ...
                break;
            case "REFRESH_PIVOT":
                this._refreshLeewisePivot(cmd.id);
                break;
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshLeewisePivots();
                break;
            ...
            case "UPDATE_LEEWISE_PIVOT_DOMAIN": {
            ...
            case "REDO": {
                ...
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) => cmd.type === "UPDATE_LEEWISE_PIVOT_DOMAIN" || cmd.type === "INSERT_PIVOT"
                );
     ...
     *
     * =LEEWISE.PIVOT(1,"expected_revenue","stage_id",2,"city","Brussels")
     * =LEEWISE.PIVOT.HEADER(1,"stage_id",2,"city","Brussels")
     * =LEEWISE.PIVOT.HEADER(1,"stage_id",2,"city","Brussels","measure","expected_revenue")
     *
     * the result is the same: ["stage_id", 2, "city", "Brussels"]
     *
     * If the cell is the result of LEEWISE.PIVOT.TABLE, the result is the domain of the cell
    ...
    getPivotDomainArgsFromPosition(position) {
        ...
        if (functionName === "LEEWISE.PIVOT.TABLE") {
        ...
        const domain = args.slice(functionName === "LEEWISE.PIVOT" ? 2 : 1);
        if (domain.at(-2) === "measure") {
            return domain.slice(0, -2);
        }
        return domain;
    ...
    /**
     * High level method computing the result of LEEWISE.PIVOT.HEADER functions.
     *
     ...
     */
    computeLeewisePivotHeaderValue(pivotId, domainArgs) {
        const dataSource = this.getters.getPivotDataSource(pivotId);
        dataSource.markAsHeaderUsed(domainArgs);
        return dataSource.computeLeewisePivotHeaderValue(domainArgs);
    }

    /**
     * High level method computing the formatted result of LEEWISE.PIVOT.HEADER functions.
     *
    ...
    getPivotHeaderFormattedValue(pivotId, pivotArgs) {
        const dataSource = this.getters.getPivotDataSource(pivotId);
        const value = dataSource.computeLeewisePivotHeaderValue(pivotArgs);
     ...
     */
    _refreshLeewisePivots() {
        for (const pivotId of this.getters.getPivotIds()) {
            this._refreshLeewisePivot(pivotId, false);
        }
    }
...
PivotUIPlugin.getters = [
    ...
    "computeLeewisePivotHeaderValue",
