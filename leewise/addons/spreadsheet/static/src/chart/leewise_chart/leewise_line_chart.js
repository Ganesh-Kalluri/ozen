...
export class LeewiseLineChart extends LeewiseChart {
...
chartRegistry.add("leewise_line", {
    match: (type) => type === "leewise_line",
    createChart: (definition, sheetId, getters) => new LeewiseLineChart(definition, sheetId, getters),
    getChartRuntime: createLeewiseChartRuntime,
    validateChartDefinition: (validator, definition) =>
        LeewiseLineChart.validateChartDefinition(validator, definition),
    transformDefinition: (definition) => LeewiseLineChart.transformDefinition(definition),
    getChartDefinitionFromContextCreation: () => LeewiseLineChart.getDefinitionFromContextCreation(),
    name: _t("Line"),
});
...
function createLeewiseChartRuntime(chart, getters) {
    ...
    for (let [index, { label, data, cumulatedStart }] of datasets.entries()) {
        ...
        if (chart.stacked) {
            // use the transparency of Leewise to keep consistency
