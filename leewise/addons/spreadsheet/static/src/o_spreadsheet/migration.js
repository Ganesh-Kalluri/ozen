...
export const LEEWISE_VERSION = 6;

const MAP = {
    PIVOT: "LEEWISE.PIVOT",
    "PIVOT.HEADER": "LEEWISE.PIVOT.HEADER",
    "PIVOT.POSITION": "LEEWISE.PIVOT.POSITION",
    "FILTER.VALUE": "LEEWISE.FILTER.VALUE",
    LIST: "LEEWISE.LIST",
    "LIST.HEADER": "LEEWISE.LIST.HEADER",
};

const dmyRegex = /^([0|1|2|3][1-9])\/(0[1-9]|1[0-2])\/(\d{4})$/i;

export function migrate(data) {
    let _data = load(data, !!leewise.debug);
    const version = _data.leewiseVersion || 0;
    if (version < 1) {
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["LEEWISE.PIVOT", "LEEWISE.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
export class LeewiseVersion extends CorePlugin {
    export(data) {
        data.leewiseVersion = LEEWISE_VERSION;
    }
}

LeewiseVersion.getters = [];

corePluginRegistry.add("leewiseMigration", LeewiseVersion);
