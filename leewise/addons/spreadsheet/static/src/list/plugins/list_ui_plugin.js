...
export class ListUIPlugin extends spreadsheet.UIPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "START":
                ...
                break;
            case "INSERT_LEEWISE_LIST": {
                const { id, linesNumber } = cmd;
                this._setupListDataSource(id, linesNumber);
                break;
            }
            case "SELECT_LEEWISE_LIST":
                this._selectList(cmd.listId);
                break;
            case "REMOVE_LEEWISE_LIST":
                if (cmd.listId === this.selectedListId) {
                    this.selectedListId = undefined;
                }
                break;
            case "REFRESH_LEEWISE_LIST":
                this._refreshLeewiseList(cmd.listId);
                break;
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshLeewiseLists();
                break;
            ...
            case "CLEAR_GLOBAL_FILTER_VALUE":
                this._addDomains();
                break;
            case "UPDATE_LEEWISE_LIST_DOMAIN": {
            ...
            case "REDO": {
                ...
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_LEEWISE_LIST_DOMAIN" || cmd.type === "INSERT_LEEWISE_LIST"
                );
     ...
     */
    _refreshLeewiseLists() {
        for (const listId of this.getters.getListIds()) {
            this._refreshLeewiseList(listId);
        }
    }
     ...
     *
     * @param {string} content Leewise list formula
     */
