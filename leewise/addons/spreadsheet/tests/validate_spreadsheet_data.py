...
import json
import re
from leewise.tests.common import TransactionCase, Form
from leewise.exceptions import ValidationError

markdown_link_regex = r"^\[([^\[]+)\]\((.+)\)$"

xml_id_url_prefix = "leewise://ir_menu_xml_id/"

leewise_view_link_prefix = "leewise://view/"


def leewise_charts(data):
    """return all leewise chart definition in the spreadsheet"""
    figures = []
    for sheet in data["sheets"]:
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet["figures"]
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("leewise_")
        ]
    return figures
...
def leewise_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like leewise://view/{... view data...}
    """
    return [
        json.loads(url[len(leewise_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(leewise_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = leewise_charts(data)
    leewise_version = data.get("leewiseVersion", 1)
    if leewise_version < 5:
...
def fields_in_spreadsheet(data):
    ...
    all_fields = chain(
        ...
        extract_fields(chart_fields, leewise_charts(data)).items(),
        extract_fields(leewise_view_fields, leewise_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartLeewiseMenusReferences", {}).values()) | {
