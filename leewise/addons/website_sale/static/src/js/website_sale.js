        ...
        if (salePage.dataset.ecomZoomAuto) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomLeewise = $image.data("zoomLeewise");
                    if (zoomLeewise) {
                        zoomLeewise.hide();
                        $image.unbind();
                    }
        ...
        if (salePage.dataset.ecomZoomClick) {
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoomleewise-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.leewiseEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.leewiseEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
