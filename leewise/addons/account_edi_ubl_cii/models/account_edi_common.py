from leewise import _, models, Command
...
from leewise.tools import float_repr, find_xml_value
from leewise.tools.float_utils import float_round
from leewise.tools.misc import formatLang
...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_invoice_ubl_cii(self, invoice, file_data, new=False):
        ...
        # This has to be done after the first import in order to let Leewise compute the taxes before overriding if needed.
    ...
    def _import_fill_invoice_line_values(self, tree, xpath_dict, invoice_line, qty_factor):
        """
        Read the xml invoice, extract the invoice line values, compute the leewise values
        ...
        It is not possible to do it in Leewise.
        ...
        if quantity_node is not None:
            ...
            if uom_xml:
                uom_infered_xmlid = [
                    leewise_xmlid for leewise_xmlid, uom_unece in UOM_TO_UNECE_CODE.items() if uom_unece == uom_xml
                ]
        ...
        for allow_charge_el in allow_charge_nodes:
            ...
            if amount is not None:
                if reason_code is not None and reason_code.text == 'AEO' and reason is not None:
                    # Handle Fixed Taxes: when exporting from Leewise, we use the allowance_charge node
