...
class InsufficientCreditDialog extends Component {
    ...
    async onWillStart() {
        ...
        const { isEnterprise } = leewise.info;
        if (errorData.trial && isEnterprise) {
            this.buttonMessage = _t("Start a Trial at Leewise");
        } else {
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "leewise.addons.iap.tools.iap_tools.InsufficientCreditError") {
