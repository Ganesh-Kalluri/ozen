...
from leewise import _lt, _
from leewise.exceptions import UserError, RedirectWarning

PROXY_ERROR_CODES = {
    "error_codabox_not_configured": _lt("CodaBox is not configured. Please check your configuration."),
    "error_connecting_iap": _lt("An error occurred while connecting to the IAP server. Please contact Leewise support."),
    "error_connecting_codabox": _lt("An error occurred while connecting to CodaBox. Please contact Leewise support."),
    ...
    "error_invalid_fidu_password": _lt("The provided password is not valid for this Accounting Firm. You must reuse the password you received from Leewise during your first connection."),
...
def get_error_msg(error):
    ...
    if error_type == 'error_connecting_codabox' and codabox_error_code:
        return CODABOX_ERROR_CODES.get(codabox_error_code, _("Unknown error %s while contacting CodaBox. Please contact Leewise support.", codabox_error_code))
    return PROXY_ERROR_CODES.get(error_type, _("Unknown error %s while contacting CodaBox. Please contact Leewise support.", error_type))
