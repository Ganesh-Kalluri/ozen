...
from leewise.tools.pdf import LeewisePdfFileReader, LeewisePdfFileWriter
    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        if self._is_invoice_report(report_ref):
            ...
            if qr_inv_ids:
                ...
                if header:
                    ...
                    for invoice_id, stream in qr_res.items():
                        qr_pdf = LeewisePdfFileReader(stream['stream'], strict=False)
                        header_pdf = LeewisePdfFileReader(header_res[invoice_id]['stream'], strict=False)

                        page = header_pdf.getPage(0)
                        page.mergePage(qr_pdf.getPage(0))

                        output_pdf = LeewisePdfFileWriter()
            ...
            for invoice_id, additional_stream in streams_to_append.items():
                ...
                writer = LeewisePdfFileWriter()
                writer.appendPagesFromReader(LeewisePdfFileReader(invoice_stream, strict=False))
                writer.appendPagesFromReader(LeewisePdfFileReader(additional_stream['stream'], strict=False))
                new_pdf_stream = io.BytesIO()
