...
leewise.loader.bus.addEventListener("module-started", (e) => {
    if (e.detail.moduleName === "@im_livechat/embed/common/livechat_service") {
        const { LivechatService, LEEWISE_VERSION_KEY } = e.detail.module;
        // Livechat sessions are deleted if they come from a different leewise version than
        ...
        patch(LivechatService.prototype, {
            async initialize() {
                try {
                    ...
                    localStorage.setItem(LEEWISE_VERSION_KEY, serverVersion);
                } catch {
