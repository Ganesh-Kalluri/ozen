...
export function usePartnerAutocomplete() {
    const keepLastLeewise = new KeepLast();
    ...
    async function autocomplete(value) {
        ...
        let leewiseSuggestions = [];
        let clearbitSuggestions = [];
        return new Promise((resolve, reject) => {
            const leewisePromise = getLeewiseSuggestions(value, isVAT).then((suggestions) => {
                leewiseSuggestions = suggestions;
            });
            ...
            const concatResults = () => {
                // Add Clearbit result with Leewise result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    const websites = leewiseSuggestions.map((suggestion) => {
                    ...
                    clearbitSuggestions.forEach((suggestion) => {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            leewiseSuggestions.push(suggestion);
                        }
                    });
                }

                leewiseSuggestions = leewiseSuggestions.filter((suggestion) => {
                    return !suggestion.ignored;
                });
                leewiseSuggestions.forEach((suggestion) => {
                    delete suggestion.ignored;
                });
                return resolve(leewiseSuggestions);
            };

            whenAll([leewisePromise, clearbitPromise]).then(concatResults, concatResults);
        });
    }
    ...
    /**
     * Use Leewise Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
     * @private
     */
    async function getLeewiseSuggestions(value, isVAT) {
        ...
        const suggestions = await keepLastLeewise.add(prom);
