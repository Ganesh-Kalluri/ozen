...
from leewise.addons.sale_amazon import utils as amazon_utils
from leewise.addons.sale_amazon.tests import common


class TestAmazon(common.TestAmazonCommon):

    @mute_logger('leewise.addons.sale_amazon.models.amazon_account')
    def test_update_marketplaces_no_change(self):
        ...
        with patch(
                'leewise.addons.sale_amazon.models.amazon_account.AmazonAccount'
    ...
    def test_sync_orders_full(self):
        ...
        with patch(
            'leewise.addons.sale_amazon.utils.make_sp_api_request',
            new=lambda _account, operation, **kwargs: common.OPERATIONS_RESPONSES_MAP[operation],
        ), patch(
            'leewise.addons.sale_amazon.models.amazon_account.AmazonAccount._recompute_subtotal',
            new=lambda self_, subtotal_, *args_, **kwargs_: subtotal_,
        ), patch(
            'leewise.addons.sale_amazon.models.amazon_account.AmazonAccount._find_matching_product',
    ...
    def test_sync_orders_partial(self):
        ...
        with patch(
            'leewise.addons.sale_amazon.utils.make_sp_api_request', new=get_sp_api_response_mock
        ):
    ...
    @mute_logger('leewise.addons.sale_amazon.models.amazon_account')
    @mute_logger('leewise.addons.sale_amazon.models.stock_picking')
    def test_sync_orders_cancel_abort(self):
        """ Test the pickings that were confirmed at leewise and then order is canceled at Amazon. """
    ...
    def test_inventory_sync_is_skipped_when_disabled(self):
        ...
        with patch(
            'leewise.addons.sale_amazon.utils.submit_feed', return_value='An_amazing_id'
        ) as mock:
    ...
    @mute_logger('leewise.addons.sale_amazon.models.amazon_account')
    @mute_logger('leewise.addons.sale_amazon.models.amazon_offer')
    ...
    def test_sync_pickings(self):
        ...
        with patch(
            ...
            'leewise.addons.sale_amazon.utils.submit_feed', new=Mock(return_value='An_amazing_id'),
        ) as mock:
            self.account._sync_orders(auto_commit=False)
            order = self.env['sale.order'].search([('amazon_order_ref', '=', '123456789')])
            picking = self.env['stock.picking'].search([('sale_id', '=', order.id)])
