...
from leewise import _, api, exceptions, fields, models
from leewise.exceptions import UserError, ValidationError
from leewise.service.model import PG_CONCURRENCY_ERRORS_TO_RETRY as CONCURRENCY_ERRORS

from leewise.addons.sale_amazon import const, utils as amazon_utils
from leewise.addons.sale_amazon.controllers.onboarding import compute_oauth_signature
...
class AmazonAccount(models.Model):
    ...
    last_orders_sync = fields.Datetime(
        help="The last synchronization date for orders placed on this account. Orders whose status "
             "has not changed since this date will not be created nor updated in Leewise.",
