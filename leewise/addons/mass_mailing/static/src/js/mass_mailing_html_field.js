...
import { getRangePosition } from '@web_editor/js/editor/leewise-editor/src/utils/utils';
import { utils as uiUtils } from "@web/core/ui/ui_service";
import { useSubEnv, status, markup } from "@leewise/owl";
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges() {
        ...
        this._pendingCommitChanges = (async () => {
            ...
            this.wysiwyg.leewiseEditor.historyPauseSteps();
            ...
            this.wysiwyg.leewiseEditor.observerUnactive('toInline');
            ...
            iframe.remove();
            this.wysiwyg.leewiseEditor.observerActive('toInline');
            const inlineHtml = editableClone.innerHTML;
            $editorEnable.addClass('editor_enable');
            this.wysiwyg.leewiseEditor.historyUnpauseSteps();
            this.wysiwyg.leewiseEditor.historyRevertCurrentStep();
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.leewise-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.leewiseEditor.observerFlush();
        this.wysiwyg.leewiseEditor.historyReset();
    ...
    async _onSnippetsLoaded() {
        ...
        $snippetsSideBar.on('click', '.o_codeview_btn', () => {
            this.wysiwyg.leewiseEditor.observerUnactive();
            $codeview.toggleClass('d-none');
            this.wysiwyg.getEditable().toggleClass('d-none');
            this.wysiwyg.leewiseEditor.observerActive();

            if ($codeview.hasClass('d-none')) {
                this.wysiwyg.setValue(this._getCodeViewValue($codeview[0]));
                this.wysiwyg.leewiseEditor.sanitize();
                this.wysiwyg.leewiseEditor.historyStep(true);
            } else {
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the Leewise editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.leewiseEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.leewiseEditor.editable.querySelector('p');
                if (p && selection) {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.leewiseEditor.resetContent($newLayout[0].outerHTML);
    ...
    async _lazyloadWysiwyg() {
        ...
        const wysiwygModule = await leewise.loader.modules.get('@mass_mailing/js/mass_mailing_wysiwyg');
