...
class TestMassMailValues(MassMailCommon):
    ...
    def test_mailing_computed_fields_domain_w_filter(self):
        ...
        filter_1, filter_2, filter_3 = self.env['mailing.filter'].create([
            ...
            {'name': 'Email based',
             'mailing_domain' : [('email', 'ilike', 'info@leewise.com')],
             'mailing_model_id': self.env['ir.model']._get('res.partner').id,
            }
        ])
        ...
        mailing.mailing_domain = "[('email', 'ilike', 'info_be@leewise.com')]"
...
Email: <a id="url5" href="mailto:test@leewise.com">test@leewise.com</a></div>""",
            'mailing_model_id': self.env['ir.model']._get('mailing.list').id,
            'reply_to_mode': 'new',
            'reply_to': self.email_reply_to,
            'contact_list_ids': [(6, 0, self.mailing_list_1.ids)],
        ...
        for contact in self.mailing_list_1.contact_ids:
            ...
            for link_info in [('url0', 'https://www.leewise.tz/my/%s' % contact.name, True),
                              ...
                              ('url5', 'mailto:test@leewise.com', False)]:
