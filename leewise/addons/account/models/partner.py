...
from leewise.tools import DEFAULT_SERVER_DATETIME_FORMAT, mute_logger
from leewise.exceptions import ValidationError, UserError
from leewise.addons.base.models.res_partner import WARNING_MESSAGE, WARNING_HELP
...
class ResPartner(models.Model):
    ...
    def _increase_rank(self, field, n=1):
        if self.ids and field in ['customer_rank', 'supplier_rank']:
            try:
                with self.env.cr.savepoint(flush=False), mute_logger('leewise.sql_db'):
                    query = sql.SQL("""
