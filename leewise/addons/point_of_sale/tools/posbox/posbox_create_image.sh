...
REPO=https://github.com/leewise/leewise.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/leewise"
...
leewise/
leewise-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/leewise
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/leewise/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
