...
function connect () {
	SERVER="${1}"
	CURRENT_SERVER_FILE=/home/pi/leewise-remote-server.conf
	...
	current_iotbox_version=$(cat "/var/leewise/iotbox_version")
	...
	if [ ${TIMEOUT_RETURN} -eq 124 ] && [ -z "${NO_AP}" ] ; then
		...
		sudo /home/pi/leewise/addons/point_of_sale/tools/posbox/configuration/wireless_ap.sh "force" &
	else
		...
		if [ ! -f "${LOST_WIFI_FILE}" ] ; then
			logger -t posbox_connect_to_server_wifi "Restarting leewise"
		fi
		...
		/home/pi/leewise/addons/point_of_sale/tools/posbox/configuration/keep_wifi_alive.sh &
	fi
