...
from leewise.exceptions import CacheMiss
...
class Field(MetaField('DummyField', (object,), {})):
    ...
    :param str group_operator: aggregate function used by :meth:`~leewise.models.Model.read_group`
    ...
    def setup(self, model):
        ...
        if not self._setup_done:
            ...
            if self.states:
                warnings.warn(f'Since Leewise 17, property {self}.states is no longer supported.')
    ...
    def groupable(self):
        """
        Return whether the field may be used for grouping in :meth:`~leewise.models.BaseModel.read_group`.
        """
    ...
    def mapped(self, records):
        ...
        over a simple call to `~leewise.models.BaseModel.mapped()` on a recordset.
        """
        if self.name == 'id':
            # not stored in cache
...
class Float(Field):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~leewise.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~leewise.fields.Float.round()` to round a float with the given precision.
    :func:`~leewise.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~leewise.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field):
    ...
    :class:`res_currency<leewise.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <leewise.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-leewise: currency may not be already initialized if it is
...
class Image(Binary):
    ...
    :param bool verify_resolution: whether the image resolution should be verified
        ...
        See :class:`leewise.tools.image.ImageProcess` for maximum image resolution (default: ``50e6``).
...
class Many2one(_Relational):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~leewise.models.Model._check_company`. Add a default company
...
class Properties(Field):
    ...
    like Leewise fields but without being independently stored in database.
    ...
    current record. The container must have a :class:`~leewise.fields.PropertiesDefinition`
...
class PropertiesDefinition(Field):
    """ Field used to define the properties definition (see :class:`~leewise.fields.Properties`
...
class Command(enum.IntEnum):
    """
    :class:`~leewise.fields.One2many` and :class:`~leewise.fields.Many2many` fields
    ...
    def create(cls, values: dict):
        ...
        In case of a :class:`~leewise.fields.Many2many` relation, one unique
        new record is created in the comodel such that all records in `self`
        are linked to the new record.

        In case of a :class:`~leewise.fields.One2many` relation, one new record
    ...
    def delete(cls, id: int):
        ...
        In case of a :class:`~leewise.fields.Many2many` relation, removing the
    ...
    def unlink(cls, id: int):
        ...
        In case of a :class:`~leewise.fields.One2many` relation, the given record
